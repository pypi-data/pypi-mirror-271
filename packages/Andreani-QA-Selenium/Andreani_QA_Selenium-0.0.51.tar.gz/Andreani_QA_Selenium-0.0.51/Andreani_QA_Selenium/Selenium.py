...
class Selenium(Functions, Parameters):
    ...
    def open_browser(self, url=None, browser=Parameters.browser, options_headless=Parameters.headless,
        ...
        self.logger.info(f"Directorio Base: {Parameters.current_path}")
        self.logger.warning(f"{self.color_message('YELLOW', 'AGUARDANDO:')} Inicio del navegador "
        ...
        try:
            if browser == "CHROME":
                if Parameters.environment_configuration == "server":
                    custom_chrome_path = r'C:\\Chromium\\chrome.exe'
    ...
    def tear_down(self):
        ...
        try:
            if Selenium.data_cache not in ([], {}):
                ...
                print("=====================Fin Cache=====================")
            self.logger.warning(f"{Selenium.color_message('YELLOW', 'AGUARDANDO:')} Se cerrará el web driver.")
            ...
            self.logger.warning(f"{Selenium.color_message('GREEN', 'REALIZADO:')} Finaliza la ejecución.")
            if Selenium.record:
    ...
    def locator_element(self, type_locator, indentify, entity=None):
        ...
        try:
            ...
            self.logger.warning(f"Se interactuo con el elemento {indentify}")
            self.logger.warning(f"{Selenium.color_message('GREEN', 'REALIZADO:')} Se detecto el elemento web "
            ...
            self.logger.warning(f"No se pudo encontrar el elemento web '{Selenium.color_message('BLUE', entity)}'"
            ...
            self.logger.warning(f"Se agoto el tiempo de busqueda intentando encontrar el elemento web "
            ...
            Functions.exception_logger(e)
            self.logger.critical(f"Ha ocurrido un error inesperado en tiempo de ejecución.")
    ...
    def get_json_file(self, file):
        ...
        try:
            with open(json_path, "r", encoding='utf8') as read_file:
                ...
                self.logger.warning(f"{Selenium.color_message('GREEN', 'REALIZADO:')} Se a cargado el respositorio de objetos "
            ...
            self.logger.warning(f"{Selenium.color_message('RED', 'ERROR:')} No se encontro "
    ...
    def alert_windows(self, accept="accept"):
        ...
        try:
            ...
            wait.until(ec.alert_is_present(), "Esperando alerta...")
...
class ElementUI(Selenium):
    ...
    def execute_action(self, action, value=None):
        ...
        self.logger.warning(f"{Functions.color_message('GREEN', 'REALIZADO:')} Se pudo {self.message} sobre "
...
class ScreenRecord(Selenium):
    ...
    @staticmethod
    def progress_bar(iterable, prefix='', suffix='', decimals=1, length=100, fill='█', print_end=''):
        """
