...
import subprocess
from AstecManager.libs.contourlib import compute_contour,compute_contour_from_junctions,compute_segmentation_from_junctions,compute_membranes_from_junctions
...
class Manager:
    ...
    def compute_contours_from_junctions(self, parameters):
        ...
        embryo_name = parameters["embryo_name"].replace('"','').replace("'","")
        embryo_folder ="./"
        folder_contour_path,folder_contour_name=compute_contour_from_junctions(embryo_folder, junction_folder_name, reducvoxelsize=float(voxel_size),
    ...
    def compute_contours(self, parameters):
        ...
        user = compute_user(parameters)
        embryo_folder ="./"
        folder_contour_path,folder_contour_name=compute_contour(embryo_folder, background_folder_name, reducvoxelsize=float(voxel_size),
                        target_normalization_max=normalisation, correction_vsize=True)
        if omero_config_file is not None:
            self.upload_on_omero(omero_config_file, embryo_name, folder_contour_name,
                                 folder_contour_path)
