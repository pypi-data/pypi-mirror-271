    ...
    def METADATA(self,
        ...
        try:
            querys = {"ORACLE":   f"""Select * from all_tab_columns where owner = '{owner}' and table_name = '{nome_tabela}' order by column_id""""",
                      ...
                      "SQLSERVER":f"""Select * from information_schema.columns where table_schema = '{owner}' and table_name = '{nome_tabela}' order by ordinal_position""",
            ...
            """
            ===========================================================================
            Aqui sera definida qual a coluna do metadado que contera o nome da coluna
            da tabela
            """
            rows = cur.fetchall()
            for col in rows:
                if database == "ORACLE":
                    column = col[2] #Posicao 2 esta o nome da coluna
                elif database == "POSTGRES":
                    pass
                elif database == "SQLITE":
                    pass
                elif database in ["MSSQL", "SQLSERVER"]:
                    column = col[3] #Posicao 3 esta o nome da coluna
                elif database == "MYSQL":
                    pass
            ...
            """
            ===========================================================================
            Se for apresentar um row id (não sao todos os bancos de dados que suportam)
            """
            if rowid:
                # -----------------------------------------
                # Banco SQLITE
            ...
            # LIMIT
            if database in ["MYSQL", "SQLITE", "POSTGRES", "REDSHIFT", "MSSQL", "SQLSERVER"]:
                if limit > 0:
                    limit = f"""\nlimit {limit}"""
                else:
                    limit = ""
