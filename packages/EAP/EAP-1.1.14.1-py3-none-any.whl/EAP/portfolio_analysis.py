...
class MultiPeriod_Univariate():
    '''
    This module is for multiperiod univariate analysis 
    '''
    def __init__(self, ret_matrix: DataFrame, sample:DataFrame):
    ...
    def fit(self, number:int, perc:list =None, percn:ndarray =None, maxlag:int=12, weight:bool=False):
        self.maxlag = maxlag
        self.weight = weight
        self.ret_matrix_row, self.ret_matrix_col = np.shape(self.ret_matrix)
        results_matrix = list()
        
        for i in range(self.ret_matrix_col):
            merge_matrix = pd.merge(self.ret_matrix.iloc[:, i], self.sample, left_index=True, right_index=True)
            model = Univariate(merge_matrix)
            model.fit(number=number, perc=perc, percn=percn, maxlag=maxlag, weight=weight)
            results_matrix.append(pd.DataFrame(model.result.T, index=pd.to_datetime(model._time), columns = np.arange(model.number+2)))
        ...
        for j in range(self.number+2):
            for i in range(self.ret_matrix_col):
                if i == 0:
                    row, col = np.shape(results_matrix[i])
                    temp_matrix = pd.DataFrame(np.zeros(row), index = results_matrix[i].index, columns=['zero'])
                    temp_add_matrix = results_matrix[i][j]
                    temp_add_matrix.name = 'number_'+ str(j) + 'col_' + str(i)
                    temp_matrix = pd.merge(temp_matrix, temp_add_matrix, left_index=True, right_index=True)
                else:
                    temp_add_matrix = results_matrix[i][j]
                    temp_add_matrix.name = 'number_'+ str(j) + 'col_' + str(i)
                    temp_matrix = pd.merge(temp_matrix, results_matrix[i][j], left_index=True, right_index=True)
            temp_matrix = 1 + temp_matrix
            temp_matrix = temp_matrix.cumprod(axis=1) - 1
#            calculate_matrix.append(temp_matrix.iloc[:, -1])
            calculate_matrix.append(temp_matrix.iloc[:, -1])
        
        self.calculate_matrix = calculate_matrix
        
        # Test
        self.average_result = np.mean(self.calculate_matrix, axis=1)
        self.ttest = np.zeros((self.number+2, 1)).T
        self.params = np.zeros((self.number+2, 1)).T
        for i in range(self.number+2):
            model = sm.OLS(calculate_matrix[i].T, np.ones((len(calculate_matrix[i]), 1)), missing='drop').fit()            
            self.ttest[0][i] = model.get_robustcov_results(cov_type='HAC', maxlags=self.maxlag, use_correction=True).tvalues[0]
            self.params[0][i] = model.get_robustcov_results(cov_type='HAC', maxlags=self.maxlag, use_correction=True).params[0]
    ...
    def factor_adjustment(self, factor:DataFrame):
        ...
        # Rows: Time
        # Columns: Groups Return
        table = self.calculate_matrix
        row_fac, col_fac = np.shape(self._factor)
        # generate anomaly: alpha 
        # generate tvalues: ttest
        alpha = np.zeros((self.number+2, 1))
        ttest = np.zeros((self.number+2, 1))
        beta = np.zeros((self.number+2, col_fac))
        beta_ttest = np.zeros((self.number+2, col_fac))
        r2 = np.zeros((self.number+2, 1))

        # factor adjusment
        for i in range(self.number+2):
            temp_result = self.calculate_matrix[i]
            temp_result.index = pd.to_datetime(temp_result.index)
            temp_mat = pd.merge(temp_result, factor, left_index=True, right_index=True).dropna()
            #model = sm.OLS(table[:,i], sm.add_constant(factor))
            # fit the model with the Newey-West Adjusment
...
class MultiPeriod_Bivariate():
    '''
    This class is for multiperiod bivariate analysis
    '''
    
    def __init__(self, ret_matrix, sample) -> None:
        if type(sample).__name__ == 'DataFrame' :
            self.sample = np.array(sample)
            self._sample_type = 'DataFrame'
            self._columns = list(sample.columns)
        ...
        self.ret_matrix = ret_matrix
        self._factor = None
        self._time = np.sort(np.unique(sample.iloc[:, 2]))
    ...
    def fit(self, number:int =4, perc_row: list=None, perc_col: list=None, percn_row: ndarray =None, percn_col: ndarray =None, weight: bool=False, maxlag: int=12, **kwargs) -> None:
        ...
        for i in range(self.ret_matrix_col):
            merge_matrix = pd.merge(self.ret_matrix.iloc[:, i], self.sample, left_index=True, right_index=True)
            model = Bivariate(merge_matrix)
            model.fit(number=number, perc_row=perc_row, perc_col=perc_col, percn_row=percn_row, percn_col=percn_col, maxlag=maxlag, weight=weight)
            results_matrix.append(model.result)
            results_time.append(model._time)
            self.num_row = model.num_row
            self.num_col = model.num_col
        
        calculate_matrix = np.zeros((self.num_row+1, self.num_col+1, len(results_time[0])))        
        for j in range(self.num_row+1):
            for k in range(self.num_col+1):
                for i in range(self.ret_matrix_col):
                    temp_group_element = pd.DataFrame(results_matrix[i][j, k], index=pd.to_datetime(results_time[i]))
                    temp_group_element.columns = ['row'+ str(j) + 'col' + str(k) + 'ret' + str(i)]
                    if i == 0:
                        row, col, len_time = np.shape(results_matrix[i])
                        temp_matrix = pd.DataFrame(np.zeros(len_time), index = pd.to_datetime(results_time[i]), columns=['zero'])
                        temp_matrix = pd.merge(temp_matrix, temp_group_element, left_index=True, right_index=True)
                    else:
                        temp_matrix = pd.merge(temp_matrix, temp_group_element, left_index=True, right_index=True)
                temp_matrix = 1 + temp_matrix
                temp_matrix = temp_matrix.cumprod(axis=1) - 1
                temp_matrix_last_col = temp_matrix.iloc[:, -1]
                temp_matrix_last_col.name = 'row'+str(j)+'col'+str(k)
#            calculate_matrix.append(temp_matrix.iloc[:, -1])
                calculate_matrix[j, k, :] = np.array(temp_matrix_last_col)
        
        self.calculate_matrix = calculate_matrix
        
        # Test
        self.average_result = np.mean(self.calculate_matrix, axis=2)
        self.ttest = np.zeros((self.num_row+1, self.num_col+1, 1))
        self.params = np.zeros((self.num_row+1, self.num_col+1, 1))
        for j in range(self.num_row+1):
            for k in range(self.num_col+1): 
                model = sm.OLS(calculate_matrix[j, k], np.ones((len(calculate_matrix[j, k]), 1)), missing='drop').fit()            
                self.ttest[j, k, 0] = model.get_robustcov_results(cov_type='HAC', maxlags=self.maxlag, use_correction=True).tvalues[0]
                self.params[j, k, 0] = model.get_robustcov_results(cov_type='HAC', maxlags=self.maxlag, use_correction=True).params[0]
    ...
    def factor_adjustment(self, factor:DataFrame):
        ...
        ttest (ndarray): The t-value of the anomaly.
        '''
        self.result = self.calculate_matrix
    ...
    def summary(self, explicit:bool =False, export:bool =False, percentage:bool =False):
        ...
        if self._factor is None :
            ...
            if self._sample_type == 'ndarray':
                ...
                table.field_names = ['Group'] + [self._columns[1] + str(i+1) for i in range(self.num_col)] + ['Diff']
            ...
            for i in range(self.num_row+1):
                ...
                for j in range(self.num_col+1):
                    ...
                    temp_tvalue.append(np.around(self.ttest[i, j, 0], decimals=3))
