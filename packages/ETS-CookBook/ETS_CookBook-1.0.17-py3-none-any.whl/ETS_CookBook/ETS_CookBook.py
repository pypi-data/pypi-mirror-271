...
def make_sankey(parameters):
    ...
    node_labels_names_only = node_parameters['labels']
    display_values = node_parameters['display_values']
    if display_values:
        values_to_add = node_parameters['values']
    ...
    node_x_positions = node_parameters['x_positions']
    node_y_positions = node_parameters['y_positions']
    node_colors = node_parameters['colors']
    ...
    link_sources = link_parameters['sources']
    link_source_indices = [
        node_labels_names_only.index(source) for source in link_sources
    ]
    link_targets = link_parameters['targets']
    link_target_indices = [
        node_labels_names_only.index(target) for target in link_targets
    ]
    value_scaling_factor = link_parameters['value_scaling_factor']
    link_values_unscaled = link_parameters['values']
    link_values = [
        link_value / value_scaling_factor
        for link_value in link_values_unscaled
    ]
    link_colors_are_source = link_parameters['link_colors_are_source']
    link_opacities = link_parameters['opacities']
    link_colors_rgba_codes = []
    for link_color_is_source, link_opacity, source, target in zip(
        link_colors_are_source, link_opacities, link_sources, link_targets
    ):
        if link_color_is_source:
