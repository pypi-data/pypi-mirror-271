...
class TilesManager(object):
    ...
    def __init__(self, **kwargs):
        ...
        wms_options -- WMS parameters to be requested (see ``Mbtiler.reader.WMSReader``)
    ...
    def tile(self, z_x_y):
        ...
        self.rendered += 1
        # Update progress bar
        with tqdm(total=self.nbtiles, desc=f'{threading.current_thread().name}', position=0, leave=True, bar_format="{desc}: {percentage:3.0f}%|{bar}| {n}/{total}") as pbar:
            pbar.update(self.rendered)
        return output
