import hashlib
import json
from abc import ABC, abstractmethod
from datetime import datetime
from functools import singledispatch
from uuid import UUID

import httpx

## copy from edwh.backend.engine
from attrs import asdict


def serialize_backend_types(instance, field, value):
    """
    Used for attrs.asdict magic.

    See https://www.attrs.org/en/stable/api.html#attr.asdict :

    value_serializer (Optional[callable]) – A hook that is called for every attribute or
    dict key/value. It receives the current instance, field and value and must return the
    (updated) value. The hook is run after the optional filter has been applied.

    web2py example:

    def demo():
        from attrs import asdict

        # return response.json(asdict(backend.pratices(search=request.vars.q)))
        return response.json(
            asdict(
                backend.pratices(search=request.vars.q),
                value_serializer=serialize_backend_types,
            )
        )

    """
    if isinstance(value, UUID):
        return str(value)
    elif isinstance(value, bytes):
        return value.decode()
    return value


def filter_backend_types(attr, value):
    return not attr.name.startswith("_")


def edwh_asdict(instance):
    return asdict(
        instance,
        value_serializer=serialize_backend_types,
        filter=filter_backend_types,
    )


### /copy


### Json converters
@singledispatch
def to_serializable(val):
    """Used by default."""
    try:
        return edwh_asdict(val)
    except:
        return str(val)


@to_serializable.register
def ts_datetime(val: datetime):
    """Used if *val* is an instance of datetime."""
    return val.isoformat()


@to_serializable.register
def ts_bytes(val: bytes):
    """Used if *val* is an instance of datetime."""
    return val.decode("utf-8")


### /json converters


class SignalSink(ABC):
    @abstractmethod
    def signal(
        self,
        name,
        signal_source,
        evidence=None,
        session_gid=None,
        user_gid=None,
        ts=None,
        related=None,
    ):
        raise NotImplementedError

    @abstractmethod
    def commit(self):
        raise NotImplementedError


class DevNullSink(SignalSink):
    "Ingores anything, but is a sink."

    def signal(
        self,
        name,
        signal_source,
        evidence=None,
        session_gid=None,
        user_gid=None,
        ts=None,
        related=None,
    ):
        pass

    def commit(self):
        pass


class ConsoleSink(SignalSink):
    "Ingores anything, but is a sink."

    def signal(
        self,
        name,
        signal_source,
        evidence=None,
        session_gid=None,
        user_gid=None,
        ts=None,
        related=None,
    ):
        print(
            f"APPLOG: {name}@{signal_source} ({session_gid}|{user_gid}) evidence: {evidence}"
        )

    def commit(self):
        pass


class NamekoServiceSink(SignalSink):
    "Asynchronous autocommit sink for sending to .prorcess_buffer on a remote service."

    def __init__(self, rpc_proxy):
        super().__init__()
        self.rpc_proxy = rpc_proxy
        self.buffer = []

    def signal(
        self,
        name,
        signal_source,
        evidence=None,
        session_gid=None,
        user_gid=None,
        ts=None,
        related=None,
    ):
        details = locals().copy()
        del details["self"]
        self.buffer.append(details)
        self.commit()

    def commit(self):
        # create a copy of the buffer just to make sure
        # that any asynch invocation will have to commit
        # at a different time.
        copy_buffer = []
        while self.buffer:
            copy_buffer.append(self.buffer.pop())
        resp = self.rpc_proxy.process_buffer.call_async(copy_buffer)
        return


class SyncHttpSink(SignalSink):
    def __init__(
        self, url, app_id, app_key, autocommit=True, ignore_certificates=False
    ):
        super().__init__()
        self.buffer = []
        self.url = url
        self.app_id = app_id
        self.app_key = app_key
        self.autocommit = autocommit
        self.ignore_certificates = ignore_certificates

    def signature_headers(self, json: str):
        if isinstance(self.app_key, int):
            self.app_key = self.app_key.to_bytes(32, "little")
        sig = hashlib.sha256(self.app_key)
        sig.update(json.encode("utf-8"))
        headers = {
            "X-edwh-client-id": self.app_id,
            "X-edwh-signature": "sha256:" + sig.digest().hex(),
        }
        return headers

    def signal(
        self,
        name,
        signal_source,
        evidence=None,
        session_gid=None,
        user_gid=None,
        ts=None,
        related=None,
    ):
        details = locals().copy()
        del details["self"]
        self.buffer.append(details)
        if self.autocommit:
            self.commit()

    def commit(self):
        if not self.buffer:
            return
        # copy from the buffer so additions to the buffer while sending
        # are not reverted by resetting self.buffer to []
        local_buffer = []
        while self.buffer:
            local_buffer.append(self.buffer.pop())
        if len(local_buffer) == 1:
            # with just one, send it straight away
            local_buffer = local_buffer[0]
        # convert to json, to_serializable handles different argument types
        data = json.dumps(local_buffer, default=to_serializable)
        # post using https since requests is not eventlet compatible
        # adding the signature based on the entire payload .
        # use self.verify == False for local development
        try:
            resp = httpx.post(
                url=self.url,
                data=data,
                headers=self.signature_headers(data),
                verify=not self.ignore_certificates,
            )
        except httpx.ReadTimeout:
            print(f"TIMEOUT committing {len(local_buffer)} signals")
            return False
        # print("sleeping")
        # eventlet.sleep(1)
        if self.ignore_certificates:
            # when developing, never ignore certs on production
            print("Webserver responded with:", resp.text)
        return True
