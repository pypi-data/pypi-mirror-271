...
def FilterType(ftype):
	"""
	ftype shall be tuple.Refer to DXF reference to learn about DXF group code.
	"""
	return win32com.client.VARIANT(pythoncom.VT_I2|pythoncom.VT_ARRAY,ftype)

def FilterData(fdata):
	"""
	fdata shall be tuple.Refer to DXF reference to learn about DXF group code.
	"""
...
class PyPCAD():
	...
	def AppPath(self):
		"""
		return PyPCAD Application path
		"""
	...
	def SendCommand(self,command):
		...
		>>>pypcad.SendCommand('circle 0,0,0 100 ') #Please notice that there is a blank following 100, meaning the end of input
		"""
	...
	def CreateNewFile(self):
		"""
		Create a new dwg file,by default name
		"""
	...
	def DeepClone(self,objects,Owner=None,IDPairs=win32com.client.VARIANT(pythoncom.VT_VARIANT, ())):
		...
		:param objects: objects needed to be deep cloned.Type: PyPCADSelectionSet(selection sets),tuple of entity object
		...
		>>result=pypcad.DeepClone((te1,),1) # Deep Clone one object,notice the naunce between (te1,)and (te1),the latter one is int.
	...
	def AddLwpline(self,*vertexCoord):
		"""
		LightWeight Poly line in active space, this method requires the group of 2D vertex coordinates(that is x,y),
	...
	def AddCircle(self,centerPnt,radius):
		"""
		add a circle in active space, centerPnt's type is Apoint
		"""
	...
	def AddArc(self,centerPnt,radius,startAngle,endAngle):
		"""
		add an arc in active space, startAngle and endAngle are both in the form of degree
		"""
	...
	def AddEllipse(self,centerPnt,majorAxis,radiusRatio):
		"""
		add ellipse in active space, the type of majorAxis is Apoint
		"""
	...
	def AddSolid(self,pnt1,pnt2,pnt3,pnt4):
		"""
		Creates a 2D solid polygon in active space. 
