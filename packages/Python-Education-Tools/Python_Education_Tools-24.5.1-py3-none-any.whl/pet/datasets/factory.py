...
#def gen_int_series(int_range_lst=[0, 100], name='mark', number=40):
    """  生成整数随机series
       ...
       int_range_lst：[start，end]
        记录条数：number，默认40
        默认名称是：mark
        返回：series    if int_range_lst is not isinstance(int_range_lst, (list, tuple)):
        int_range_lst = [0, 100]
...
def gen_float_series(float_range_lst=[0, 100, 2], number=40):
    ...
    # 创建 DataFrame
    df = pd.Series(random_floats)

    return df

'''

def gen_date_time_series(period=['2020-2-24 00:00:00', '2022-12-31 23:59:59'], number=40, frmt="%Y-%m-%d %H:%M:%S"):
    """
...
def gen_date_time_series(period=['2020-02-24 00:00:00', '2022-12-31 23:59:59'], number=40):
    ...
    # 生成随机日期时间
    random_datetime = pd.date_range(start=start_datetime, end=end_datetime, periods=number)
    # 创建 Series
    df = pd.Series( random_datetime)
    print('who are U')
    return df
