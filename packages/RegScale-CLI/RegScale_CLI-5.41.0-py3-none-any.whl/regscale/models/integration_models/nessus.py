...
class NessusReport(BaseModel, IntegrationIssue):
    ...
    def process_assets(
        ...
        :param List[dict[Any,Any]] cpe_list: A list of CPE elements
        :param List[Asset] existing_assets: A list of existing assets
    ...
    def process_nessus_file(
    ...
    ) -> Tuple[Set[Issue], List[Asset], Set[int]]:
        """
        ...
        :return: A tuple of sets of issues and existing assets
        :rtype: Tuple[Set[Issue], List[Asset], Set[int]]
        """
        ...
        existing_assets_in_scan: Set[int] = set()
        inserted_scans = []
        ...
        new_issues, existing_issues = NessusReport.generate_issues(app, logger, regscale_ssp_id, vulns_to_post)
    ...
    @staticmethod
    def process_files(file_collection, regscale_ssp_id, app, logger):
        insert_issues: Set[Issue] = set()
        assets_scanned: Set[int] = set()
        for file in file_collection:
            (
                new_issues,
                existing_tenable_issues,
                existing_asset_in_scan,
            ) = NessusReport.process_nessus_file(file, regscale_ssp_id, app, logger)
            insert_issues.update(new_issues)
            message = "Closing issues in RegScale that are resolved based on this Nessus scan."
            NessusReport.clear_issues(
                existing_issues=existing_tenable_issues,
                existing_assets=existing_asset_in_scan,
                scan_issues=new_issues,
                message=message,
                model_id=regscale_ssp_id,
                module="securityplans",
                issue_function=NessusReport.issue_resolution,
            )

            assets_scanned.update(existing_asset_in_scan)
        return insert_issues, assets_scanned

    @staticmethod
    def issue_resolution(**kwargs: dict[str, Any]) -> List[Issue]:
        """
        Function to Handle Issue Resolution for Tenable Nessus

        :param dict[str, Any] kwargs: Additional keyword arguments
        :return: A list of issues
        :rtype: List[Issue]
        """
        module_id = kwargs.get("model_id", 0)
        module = kwargs.get("module", "securityplans")
        module_assets: List[Asset] = Asset.get_all_by_parent(module_id, module)
        scan_assets: Set[int] = kwargs.get("existing_assets", [])
        existing_issues: List[Issue] = kwargs.get("existing_issues", [])
        scan_issues: List[Issue] = kwargs.get("scan_issues", [])
        clear_issues: Set[Issue] = set()
        for iss in existing_issues:
            matched = [asset for asset in module_assets if asset.name == iss.assetIdentifier]
            if matched and matched[0].id in scan_assets:
                # If the asset exists in the scan, and we have an open issue with that asset, close the issue
                if (
                    iss.tenableNessusId not in [issue.tenableNessusId for issue in scan_issues]
                    and iss.status != "Closed"
                    and iss.tenableNessusId
                ):
                    iss.status = "Closed"
                    clear_issues.add(iss)
        return list(clear_issues)
    ...
    def process_nessus_files(regscale_ssp_id: int, folder_path: Path) -> None:
        ...
        insert_issues, _ = NessusReport.process_files(file_collection, regscale_ssp_id, app, logger)

        # Save
        issues = Issue.bulk_insert(app, issues=list(insert_issues))
    ...
    def generate_issues(
    ...
    ) -> Tuple[Set[Issue], List[Issue]]:
        """
        Generate new and return existing issues from a Nessus file
        ...
        :rtype: Tuple[Set[Issue], List[Issue]]
        """
        ...
        return new_issues, tenable_issues
