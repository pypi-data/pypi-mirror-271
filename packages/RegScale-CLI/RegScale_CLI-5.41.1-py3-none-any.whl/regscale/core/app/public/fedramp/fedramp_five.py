    ...
    for index, part in enumerate(parts):
        ...
        try:
            option = ImplementationOption(
                name=part.get("name", "Default Name"),
...
def create_leveraged_authorizations(services: List[LeveragedService], user_id: str, ssp_id: int, approval_date: str):
    ...
    """
    existing_authorizations: List[LeveragedAuthorization] = LeveragedAuthorization.get_all_by_ssp(parent_id=ssp_id)
    ...
    for service in services:
        ...
        if existing_service:
            ...
            existing_service.dataTypes = service.data_types
            existing_service.authorizedUserTypes = service.authorized_user_authentication
            existing_service.servicesUsed = ""
            existing_service.impactLevel = service.impact_level
            existing_service.natureOfAgreement = service.agreement_type
            existing_service.tenantsId = 1
            existing_service.save()
        else:
            LeveragedAuthorization(
                title=service.fedramp_csp_name,
                fedrampId=service.auth_type_fedramp_id,
                ownerId=user_id,
                ...
                authorizedUserTypes=service.authorized_user_authentication,
                impactLevel=service.impact_level,
                natureOfAgreement=service.agreement_type,
                tenantsId=1,
            ).create()
