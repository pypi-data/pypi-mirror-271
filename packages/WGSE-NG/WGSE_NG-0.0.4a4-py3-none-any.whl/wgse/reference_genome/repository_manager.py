...
class RepositoryManager:
    def __init__(
        ...
        # Index genomes by the MD5 and length of all their sequences
        self._genomes_by_md5: dict[str, list[Genome]] = self._group_genomes(
            lambda g: self._sequences_md5(g.sequences), self._genomes
        )
        self._genomes_by_length: dict[int, list[Genome]] = self._group_genomes(
            lambda g: self._sequences_length(g.sequences), self._genomes
        )
    ...
    def _group_genomes(self, criteria, genomes : list[Genome]):
        grouped: dict[str, list[Genome]] = {}
        incomplete = []
        for genome in genomes:
            evaluated_criteria = criteria(genome)
            if evaluated_criteria is None:
                incomplete.append(genome)
                continue
            if evaluated_criteria not in grouped:
                grouped[evaluated_criteria] = []
            grouped[evaluated_criteria].append(genome)
        return grouped
