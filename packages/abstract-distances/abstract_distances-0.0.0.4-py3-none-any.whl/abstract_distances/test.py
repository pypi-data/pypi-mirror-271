...
def findItter(js_obj,value):
    for key,values in js_obj.items():
        if values == value:
            return key
...
def get_closest_headers(df,*args,**kwargs):
    ...
    closest_js={str(key):"" for key in convert_list}
    headers = df.columns.tolist()

    closest_js,convert_list,headers = find_commons_dna(convert_list,headers,closest_js)
...
def choose_parser(header,df,query):
    ...
    df = get_df(df)
    choices = get_closest_headers(df,list(parsers.keys()))
    pattern = parsers.get(findItter(choices,header))
    if pattern:
        query = extract_emails(query, pattern)
    else:
