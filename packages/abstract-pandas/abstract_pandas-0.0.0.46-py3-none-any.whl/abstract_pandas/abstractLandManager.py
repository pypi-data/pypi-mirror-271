...
from abstract_pandas.excel_module import get_df,create_dataframe,get_row,get_cell_value,gdf
from abstract_pandas.file_utils import safe_excel_save
from abstract_pandas.functions import haversine
from shapely.geometry import Polygon,MultiPolygon
import os
class landManager:
    _instance = None
    def __init__(self,directory_js=None,default_polygon=None,column=None,value=None,index=1):
    ...
    def check_geo_json(self,dir_name):
        geoJsonPath = self.file_manager.get(dir_name,{}).get('geojson')
        if geoJsonPath == None or not os.path.isfile(geoJsonPath):
            shp_file = self.file_manager.get(dir_name,{}).get('shp')
            if shp_file and os.path.isfile(shp_file):
                ...
                geoJsonPath = os.path.join(directory,f"{fileName}.geojson")
                safe_excel_save(gdf,geoJsonPath)
                self.file_manager[dir_name]['geojson'] = geoJsonPath
        return geoJsonPath
...
def get_shx(dir_name=None,file_path="shx"):
    land_mgr = landManager.get_instance()
    return land_mgr.get_contents(dir_name,file_path)
def get_geojson(dir_name=None,file_path="geojson"):
    land_mgr = landManager.get_instance()
    return land_mgr.get_contents(dir_name,file_path)
