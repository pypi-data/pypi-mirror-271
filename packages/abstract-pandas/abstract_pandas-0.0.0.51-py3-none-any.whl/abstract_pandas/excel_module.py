...
async def filter_dataframe(df, column,query,caseOption=False,substringOption=False,inverseOption=False):
    ...
    series = is_inverse(condition,inverse=inverseOption)
    return df[series]
async def filter_excel(df,query,column,range_option=False,caseOption=False,substringOption=False,inverseOption=False,distanceOption=False,geo_location_refference=None):
