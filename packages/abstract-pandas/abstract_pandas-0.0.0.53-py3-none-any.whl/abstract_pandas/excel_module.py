...
def get_df(source=None, nrows=None, skiprows=None, condition=None, indices=None):
    ...
    if isinstance(source, str) and os.path.isfile(source):
        ...
        try:
            ...
            elif file_ext in ['.shp', '.cpg', '.dbf', '.shx', '.prj','.geojson']:
...
async def filter_dataframe(df, column,query,caseOption=False,substringOption=False,inverseOption=False):
    ...
    series = is_inverse(condition,inverse=inverseOption)
    return df[series]
async def filter_excel(df,query,range_option=False,caseOption=False,substringOption=False,inverseOption=False,distanceOption=False,geo_location_refference=None):
    logging.info(f"recieved excel: {file_path}")
    df = get_df(df)
    if not isinstance(query,list):
        query = split_and_clean_lines(query)
    query=make_list(query)
    val = get_values(values)
    if distanceOption:
    ...
    if range_option:
        ...
        series = await get_range(df,column,query,inverseOption=inverseOption,caseOption=caseOption,substringOption=substringOption)
    else:
        series = await filter_dataframe(df,column,query,caseOption=caseOption,substringOption=substringOption,inverseOption=inverseOption)
    return df
