...
1) Svatunek, D., & Houk, K. N. (2019). autoDIAS: a python tool for an automated distortion/interaction activation strain analysis. In Journal of Computational Chemistry (Vol. 40, Issue 28, pp. 2509–2515). Wiley. https://doi.org/10.1002/jcc.26023
2) van Zeist, W.-J., & Bickelhaupt, F. M. (2010). The activation strain model of chemical reactivity. In Organic &amp; Biomolecular Chemistry (Vol. 8, Issue 14, p. 3118). Royal Society of Chemistry (RSC). https://doi.org/10.1039/b926828f
...
Note) Both `Abbr.` and `Full Name` are supported.
""")
...
def load_model(model:str="b973c")->torch.jit.ScriptModule:
  ...
  Parameters
  ----------
    - model(str) : Name of ML-potential model. Default is b973c. \n
  Check available names via `aimDIAS.models()`
...
def DIASparser(resultDictionary:dict, fragType:str, energyType:str=None, relative_idx:None|str|int=None, unit:str="kcal/mol"):
  ...
  ----------
    ...
    - unit (str, optional) : The unit for the energy values ("kcal/mol", "kJ/mol", "Hatree", or "eV"). Default is "kcal/mol".
...
def trajDIAS(trajFile:str, fragments_params: list | dict, model:str="b973c", resultSavePath:str="./", save_kws:dict={}):
  """
  Description
  -----------
  ...
  ----------
    ...
    - fragments_params (list | dict) : Fragment parameters. Details are `fragments_params_processing()` docs.
    - model (str, optional) : Name of the AIMNet2 model to use for energy calculations. Default is "b973c".
    - resultSavePath (str, optional) : The path to save the DIAS results. Default is current directory.
  ...
  for IRC_idx, xyzString in enumerate(read_traj(trajFile, returnString=True)):
    ...
    trajDIASresult[IRC_idx] = DIAS(xyzString, fragments_params = fragments_params, clear_log=True)
  json_dump(trajDIASresult=trajDIASresult, trajFile=trajFile, **save_kws)
  print("aimDIAS CALCULATION TERMINATED NORMALLY")
  return trajDIASresult

# old geometric_parameter
'''
def geometric_parameter(trajFile:str, geo_param:dict)->tuple:
  ...
  (3.0, 3.1, 3.2, ...)
  """
  paramType, param = tuple(geo_param.items())[0] # indices in param_1 starts with 1

  try: # tuple or list is processed to start indexing from 0
    param = tuple(map(lambda starts_with_1 : starts_with_1 - 1, param ))
  ...
  match paramType:
    ...
    case "distance":
      ...
      if type(param) == str:
        param_tuple = list(map(lambda str : int(str) - 1, re.split("\s+", param)))
        paramNum = len(param_tuple)
        if paramNum == 2:
          return tuple(map(lambda Atoms : Atoms.get_distance(*param_tuple), read_traj(trajFile)))
        elif paramNum != 2 :
          raise ValueError(f"geo_param expected 2 but {paramNum}, {tuple(geo_param.values())[0]}")
      # distance | [ list | tuple ]   ex) geo_param = { "distance" : [1,2] }
      elif type(param) == list or type(param) == tuple:
        if len(param) == 2:
          return tuple(map(lambda Atoms : Atoms.get_distance(*param), read_traj(trajFile)))
        elif len(param) != 2 :
          raise ValueError(f"geo_param expected 2 but {len(param)}, {tuple(geo_param.values())[0]}")
      else:
    ...
    case "angle":
      ...
      if type(param) == str:
        ...
        paramNum = len(param_tuple)
        if paramNum == 3:
          return tuple(map(lambda Atoms : Atoms.get_angle(*param_tuple), read_traj(trajFile)))
        elif paramNum != 3 :
          raise ValueError(f"geo_param expected 3, got {paramNum}, {tuple(geo_param.values())[0]}")
      # angle | [ list | tuple ]   ex) geo_param = { "angle" : [1,2,3] }
      elif type(param) == list or type(param) == tuple:
        if len(param) == 3:
          return tuple(map(lambda Atoms : Atoms.get_angle(*param), read_traj(trajFile)))
        elif len(param) != 3 :
          raise ValueError(f"geo_param expected 3, got {len(param)}, {tuple(geo_param.values())[0]}")
      else:
        raise ValueError("Invalid geo_param value")
    ...
    case "dihedral":
      ...
      if type(param) == str:
        ...
        paramNum = len(param_tuple)
        if paramNum == 4:
          return tuple(map(lambda Atoms : Atoms.get_dihedral(*param_tuple), read_traj(trajFile)))
        elif paramNum != 4 :
          raise ValueError(f"geo_param expected 4, got {paramNum}, {tuple(geo_param.values())[0]}")
      # dihedral | [ list | tuple ]   ex) geo_param = { "dihedral" : [1,2,3,4] }
      elif type(param) == list or type(param) == tuple:
        if len(param) == 4:
          return tuple(map(lambda Atoms : Atoms.get_dihedral(*param), read_traj(trajFile)))
        elif len(param) != 4 :
          raise ValueError(f"geo_param expected 4, got {len(param)}, {tuple(geo_param.values())[0]}")
      else:
        raise ValueError("Invalid geo_param value")

    case _:
      raise ValueError(f"Invalid param type : {param} | Supported types : 'distance', 'angle', 'dihedral'") 
'''
...
def xaxis_formatter(trajFile:str, geo_param:dict):
  ...
  x_axis_format = f'{paramType}({symbol_index}) in {axis_unit[paramType]}'
  return paramType, x_axis_format


def aimDIAS_run(trajFile:str, fragments_params: list | dict, mode:str="calculation", DIAresultPath:str="aimDIAS_result.json", axis_type:str="irc", geo_param:dict | None =None, model:str="b973c", unit:str="kcal/mol", include_fragments=False, relative_idx:None|str|int=0, plot_highlights:dict={"marker": False, "linestyle": True}, resultSavePath:str="./", horizontal_line:bool=True, save_kws:dict={})->None:
  """
  Description
  -----------
  ...
  ----------
    ...
    - unit (str, optional) : The unit for energy values. Default is "kcal/mol".
    ...
    - resultSavePath (str, optional) : The path to save the plot. Default is current directory.
  ...
  # ylabel
  ylabel_ = f"{'Rel. ' if relative_idx is not None else ''}Energy in {unit}"
  ...
  marker_list = ["+", "x", "4"]
  linestyle_list = ["-", "-.", "--"]
  for idx, Energy in enumerate(default_energy_componet):
    tmp_energy = DIASparser(DIASresult, fragType="molecule", energyType = Energy, relative_idx=relative_idx) 
    plt.plot(geo_param_axis_, tmp_energy, label=f"E_{Energy[:3]}",color=colors_list[idx], marker=marker_list[idx] if mk_bool else None, linestyle=linestyle_list[idx] if ls_bool else None)
  ...
  if include_fragments == True:
    ...
    for idx, fragment in enumerate(fragment_names):
      frag_dist_energy =  DIASparser(DIASresult, fragType=fragment, energyType = "distortion", relative_idx=relative_idx)
