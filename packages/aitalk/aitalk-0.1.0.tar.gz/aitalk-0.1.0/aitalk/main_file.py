...
class TextToSpeech:
    ...
    def speak(self, text):
        ...
        start_time = time  # Record the time before sending the request
            ...
            for chunk in r.iter_content(chunk_size=1024):
                if chunk:
                    if first_byte_time is None:  # Check if this is the first chunk received
                        first_byte_time = time  # Record the time when the first byte is received
    ...
    async def main(self):
        ...
        while True:
            ...
            with open("conv.txt", "a") as f:
                f.write(f"{self.transcription_response}\n{response}\n")
            ...
            tts = TextToSpeech()
            tts.speak(response)
            print(f"\n --- USER: {self.transcription_response}\n --- Assistant: {response}\n")
            # Reset transcription_response for the next loop iteration
            self.transcription_response = ""


if __name__ == "__main__":
    asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())  # Only necessary for Windows
