...
class SnPipeline:
    ...
    def _transform_photometry(self, X: np.ndarray, X_err: np.ndarray = None, just_grizy: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        ...
        all_mu = np.array([-2.87849839, -2.79518071, -2.78359176, -2.63765973, -2.66592091,
                           -1.33953981, 0.06383305, 0.67075717, 1.34771103, 0.91141463,
                           1.33652476, 1.2116005, 1.32755404, -2.41376848, -2.20197162,
                           -1.97797089, -3.03436027, -3.03643454])  # means of the train photometry
        all_std = np.array([0.63162979, 0.70187988, 0.68387317, 0.72578209, 0.7777774,
                            0.88136263, 1.62222597, 2.06392269, 0.43210206, 1.36265575,
                            0.50050502, 0.43821229, 0.46058673, 2.21595863, 2.13270738,
                            2.02278681, 0.67246779, 0.70040306])  # stds of the train photometry

        # Log transform, then normalize
        if X_err is not None:
            X_err = np.abs(X_err[:, : n_bands] / (X[:, : n_bands] * np.log(10)))
            X_err = X_err / all_std[: n_bands]
        X = np.log10(X)
        X = (X - all_mu[: n_bands]) / all_std[: n_bands]
    ...
    def _inverse_tranform_properties(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        mu = np.array([8.87088133, -0.46037044, 0.58991822]) # means of the train photometry
        std = np.array([1.08494612, 1.04024203, 0.2700682 ]) # means of the train photometry

        return X * std + mu


    def predict_host_properties(self, X_grizy: np.ndarray, X_grizy_err: np.ndarray = None, n_resamples: int = 10) -> np.ndarray:
        ...
        Returns:
            The host properties of the given galaxies as a np.ndarray.
        """
        ...
        if X_grizy_err is not None:
            ...
            self.host_props = np.median(all_predictions_stacked, axis=0)
        else:
            if domain_transfer:
                X_grizy = self._transfer_domain(torch.from_numpy(X_grizy))

            # Predict host properties
            self.host_props = self.property_predicting_net(torch.from_numpy(X_grizy)).detach().numpy()
    ...
    def predict_classes(self, X_grizy: np.ndarray, angular_sep: np.ndarray, X_grizy_err: np.ndarray = None, n_resamples: int = 10):
        ...
        """
        # Get the host props
        host_props_norm = self.predict_host_properties(X_grizy, X_grizy_err, n_resamples)
        host_props = self._inverse_tranform_properties(host_props_norm)  # un-normalize the properties
        host_props = np.hstack((angular_sep.reshape(-1, 1), host_props))  # in order: (angular separation, mass, SFR, redshift)
