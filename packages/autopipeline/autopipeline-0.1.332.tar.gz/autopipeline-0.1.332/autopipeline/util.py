...
def check_alias(enum, all_description, new_description, verbose, client, gpt4):
    ...
    if gpt4:
        ...
        if verbose:
            ...
            print("VERBOSE:"+"Cost for 'check_alias': $", current_pize)
            autopipeline.cost += current_pize
            print("VERBOSE:"+"Accumulated cost: $", autopipeline.cost)
    else:
        response = client.chat.completions.create(
...
def format_with_verbose(df):
    # Convert the DataFrame to a list of strings per row
    row_strings = []
    for index, row in df.iterrows():
        row_string = "VERBOSE: " + str(row).replace("\n", "\n          ")  # Maintain indent for internal lines
