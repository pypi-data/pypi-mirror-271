...
class ChatSpecialTokens:
    ...
    @classmethod
    def from_dict(cls, chat_special_tokens_dict: Dict[str, str]):
        ...
        chat_special_tokens_fields = cls.__dataclass_fields__
        for field_meta in chat_special_tokens_fields.values():
            if field_meta.name not in chat_special_tokens_dict:
                raise ACFTUserException._with_error(
                    AzureMLError.create(
                        ACFTUserError,
                        pii_safe_message=(
                            f"`{field_meta.name}` missing in :param chat_special_tokens in finetune config. "
                            "Please update the finetune config and try again."
                        )
                    )
                )
        return cls(**chat_special_tokens_dict)
    ...
    def get_chat_tokens(self) -> List[str]:
        """Get the chat tokens."""
...
class ChatCompletionDataset(AzuremlDataset, AzuremlValidatorMixin):
    ...
    def get_collation_function(self) -> Callable:
        ...
        response_template = self.chat_special_tokens.response_template
        instruction_template = self.chat_special_tokens.instruction_template
