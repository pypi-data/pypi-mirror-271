...
ACAM_DOUBLE_FLOAT_MATCHING = Template(
    r"""
        $cam_type MSB_LO = cam[4 * (cam_row_index * columns + i)];
        $cam_type LSB_LO = cam[4 * (cam_row_index * columns + i) + 1];
        $cam_type MSB_HI = cam[4 * (cam_row_index * columns + i) + 2];
        $cam_type LSB_HI = cam[4 * (cam_row_index * columns + i) + 3];
        $inputs_type Q_MSB = inputs[2 * (input_row_index * columns + i)];
        $inputs_type Q_LSB = inputs[2 * (input_row_index * columns + i) + 1];

        /* nan = don't care */
        if (!(
            (isnan(MSB_LO) || isnan(LSB_HI) || Q_MSB >= MSB_LO + 1 || Q_LSB >= LSB_LO) &&
            (isnan(MSB_LO) || Q_MSB >= MSB_LO) &&
            (isnan(MSB_HI) || isnan(LSB_HI) || Q_MSB < MSB_HI || Q_LSB < LSB_HI) &&
            (isnan(MSB_HI) || Q_MSB < MSB_HI + 1)
        )) {
            $on_mismatch
        }
"""
...
ACAM_DOUBLE_INT_MATCHING = Template(
    r"""
        ...
        /* < 0 = don't care */
        if (!(
            (MSB_LO < 0 || LSB_HI < 0 || Q_MSB >= MSB_LO + 1 || Q_LSB >= LSB_LO) &&
            (MSB_LO < 0 || Q_MSB >= MSB_LO) &&
            (MSB_HI < 0 || LSB_HI < 0 || Q_MSB < MSB_HI || Q_LSB < LSB_HI) &&
            (MSB_HI < 0 || Q_MSB < MSB_HI + 1)
        )) {
            $on_mismatch
        }
"""
