    ...
    def extract_stm_address(self, role_id: int) -> str:
        ...
        for a_task in self._tasks:
            if a_task.task_type == ClusterDetails.TaskInfo.TaskType.WRK:
                for a_map in a_task.task_map:
                    if a_map.task_id.task_id == role_id:
                        # Streamer runs on the same host as its paired WRK and only communicates
                        # with the WRK, so we spin it up on the same host.
                        # TODO: We should use task_comm_port
                        # return f"localhost:{a_map.address_book.task_comm_port}"
