    ...
    def __call__(cls, *args, **kwargs):
        if cls not in cls.instance:
            ...
            raise RuntimeError(
                ...
                f"or use cstorch.backend().torch_device to access the current backend's torch device."
            )
...
def backend(backend_type: str = None, *args, **kwargs):
    ...
    if backend_type == None:
        ...
        if len(args) != 0 or len(kwargs) != 0:
            raise TypeError(
                ...
                "or call with no arguments to get the instance of the current backend, if it exists."
            )
