...
class StreamerServicer(
    ...
    def __init__(
        ...
        self._per_box_batch_size: Optional[int] = None
        ...
        self._configure_cluster(cluster_details, role_id)
    ...
    def UnaryLoad(self, request: LoadRequest, context):
        ...
        self._per_box_batch_size = sample_spec_json.pop("per_box_batch_size")
    ...
    def UnaryRun(self, request: RunRequest, context):
        ...
        self.logger.info(
            ...
            f"\n\tcheckpoint_freq={request.checkpoint_freq}"
            ...
            f"\n\tinitial_state={request.dataloader.id != ''}"
        )
        ...
        if (
            ...
            if request.checkpoint_freq:
                ckpt_steps = min(request.checkpoint_freq, total_steps)
                assert ckpt_steps > 0

                # Allow checkpointing at the initial and last steps
                checkpoint_schedule = itertools.chain(
                    range(0, total_steps, ckpt_steps), [total_steps]
                )
            else:
                ...
                checkpoint_schedule = [total_steps]
            ...
            self._async_generators[self._curr_generator_id].start(
                ...
                batch_size=self._per_box_batch_size,
    ...
    def _configure_cluster(self, cluster_details, role_id):
        cluster_details_parser = ClusterDetailsParser(cluster_details)
        wse_id, _ = cluster_details_parser.extract_wse_details(
            ClusterDetails.TaskInfo.TaskType.WRK, role_id
        )
        ApplianceWorkerClusterResolver.configure(
            rank=role_id,
            num_tasks=cluster_details_parser.extract_num_workers(),
            wse_id=wse_id,
            num_csx=cluster_details_parser.extract_num_csx(),
            num_workers_per_csx=cluster_details_parser.extract_num_workers(
                wse_id
            ),
        )
