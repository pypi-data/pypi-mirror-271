...
class MegaBatcher(DataPipeStage, log.ClassLogger):
    ...
    def __init__(self, source: DataPipeStage, block_size: int):
        """Constructs a `MegaBatcher` instance.

        Args:
            source: The source data pipe to cycle through.
            block_size: Number of samples to put in each block.
        """
        self._source = source
    ...
    def _get_mega_batch(self) -> Dict[str, np.ndarray]:
        ...
        for name, buffered_batches in self._buffered_data.items():
            if buffered_batches[0].shape[0] != self._block_size:
                ...
                buffered_batches[0] = buffered_batches[0][split_indices[1] :]
                if buffered_batches[0].shape[0] == 0:
                    del buffered_batches[0]
            else:
                mega_batch[name] = buffered_batches[0]
                del buffered_batches[0]
...
class DataLoaderCheckpointer(DataPipeStage, log.ClassLogger):
    ...
    def __init__(
        ...
        self._checkpoint_step_map: Dict[
            int, List[int]
    ...
    def _create_checkpoint_step_map(
        ...
        """
        ckpt_step_map = defaultdict(list)
        for ckpt_step in checkpoint_schedule:
            ...
            ckpt_step_map[worker_iter].append(ckpt_step)

        for steps in ckpt_step_map.values():
            steps.sort()

        return dict(ckpt_step_map)
...
class RateProfiler(DataPipeStage, log.ClassLogger):
    ...
    def __iter__(self):
        ...
        self.logger.info(
            f"Creating iterator took {tracker.elapsed_seconds()} seconds"
        )
