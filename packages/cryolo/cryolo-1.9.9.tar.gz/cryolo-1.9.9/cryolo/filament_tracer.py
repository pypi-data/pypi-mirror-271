...
class FilamentTracer:
    ...
    def trace_filaments(self):
        ...
        # NMS for filaments:
        filaments = nms_for_filaments(filaments, self.iou_thresh, self.min_number_boxes)
...
import typing
def nms_for_filaments(filaments: typing.List[utils.Filament], iou_thresh=0.3, min_num_boxes=1):
    # NMS for filaments
    # sort by length:

    filaments = sorted(filaments, key=lambda x: -len(x.boxes))
    for fil in filaments:
        fil.meta['boxes_to_delete'] = set()
    for i in range(len(filaments)):
        for j in range((i + 1), len(filaments)):
            if i != j:
                ...
                for box_i, box in enumerate(filaments[short].boxes):
                    ...
                    if np.any(ious):
                        filaments[short].meta['boxes_to_delete'].add(box_i)

    new_set_filaments = []
    for fil in filaments:
        if len(list(fil.meta['boxes_to_delete']))>0:
            splitted_filament = split_filamant2(
                                    fil, list(fil.meta['boxes_to_delete'])

                                )
            new_set_filaments.extend(splitted_filament)
        else:
            new_set_filaments.append(fil)

    return new_set_filaments
...
def split_filamant2(filament, split_indices):
    """
    ...
    segments = []
    split_indices.sort()
    box_index_to_keep = [box_i for box_i,_ in enumerate(filament.boxes) if box_i not in split_indices]
    new_segment = []
    for box_to_keep in box_index_to_keep:
        if new_segment:
            if (box_to_keep-new_segment[-1])==1:
                new_segment.append(box_to_keep)
            else:
                seg_boxes = [box for box_i, box in enumerate(filament.boxes) if box_i in new_segment]
                segments.append(Filament(seg_boxes))
                new_segment = [box_to_keep]
        else:
            new_segment.append(box_to_keep)
