...
def merge_filaments_3d(filaments, iou_threshold, merge_threshold, window_radius, box_distance, partial = False):
    ...
    def merge_filaments(filaments, window_size, box_distance):
        ...
        boxes3d = []
        mindot = min(dotproducts)-window_size/2
        maxdot = max(dotproducts)+window_size/2
        N= max(1,int((maxdot-mindot)/(0.5*box_distance)))
        sample_points = np.linspace(mindot,maxdot,N)
