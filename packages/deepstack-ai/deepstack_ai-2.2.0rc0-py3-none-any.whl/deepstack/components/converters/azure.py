...
from deepstack.components.converters.utils import get_bytestream_from_source, normalize_metadata
...
class AzureOCRDocumentConverter:
    ...
    Usage example:
    ```python
    from deepstack.components.converters import AzureOCRDocumentConverter
    ...
    def _convert_tables_and_text(self, result: "AnalyzeResult", meta: Optional[Dict[str, Any]]) -> List[Document]:
        """
        Converts the tables and text extracted by Azure's Document Intelligence service into Deepstack Documents.
    ...
    def _convert_tables(self, result: "AnalyzeResult", meta: Optional[Dict[str, Any]]) -> List[Document]:
        """
        Converts the tables extracted by Azure's Document Intelligence service into Deepstack Documents.
    ...
    def _convert_to_single_column_text(
        ...
        """
        This converts the `AnalyzeResult` object into a single Deepstack Document.
