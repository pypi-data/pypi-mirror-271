...
class ComponentMeta(type):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        # `__deepstack_output__` is already populated, no need to do anything.
        if not hasattr(instance, "__deepstack_output__"):
            # If that's not the case, we need to populate `__deepstack_output__`
            #
            ...
            instance.__deepstack_output__ = Sockets(
        ...
        if not hasattr(instance, "__deepstack_input__"):
            instance.__deepstack_input__ = Sockets(instance, {}, InputSocket)
        ...
        for param in list(run_signature.parameters)[1:]:  # First is 'self' and it doesn't matter.
            if run_signature.parameters[param].kind not in (
                ...
                instance.__deepstack_input__[param] = InputSocket(**socket_kwargs)
        ...
        instance.__deepstack_added_to_pipeline__ = None
        ...
        is_variadic = any(socket.is_variadic for socket in instance.__deepstack_input__._sockets_dict.values())
        if not is_variadic and cls.__deepstack_is_greedy__:
            logger.warning(
...
def _component_repr(component: Component) -> str:
    ...
    if pipeline := getattr(component, "__deepstack_added_to_pipeline__"):
    ...
    # has the __deepstack_input__ and __deepstack_output__ attributes at this point
    return f"{result}\n{component.__deepstack_input__}\n{component.__deepstack_output__}"  # type: ignore[attr-defined]
...
class _Component:
    ...
    def set_input_type(self, instance, name: str, type: Any, default: Any = _empty):  # noqa: A002
        ...
        if not hasattr(instance, "__deepstack_input__"):
            instance.__deepstack_input__ = Sockets(instance, {}, InputSocket)
        instance.__deepstack_input__[name] = InputSocket(name=name, type=type, default_value=default)
    ...
    def set_input_types(self, instance, **types):
        ...
        """
        instance.__deepstack_input__ = Sockets(
    ...
    def _component(self, cls, is_greedy: bool = False):
        ...
        setattr(cls, "__deepstack_is_greedy__", is_greedy)
