...
from deepstack.core.serialization import DeserializationCallbacks, component_from_dict, component_to_dict
from deepstack.core.type_utils import _type_name, _types_are_compatible
from deepstack.marshal import Marshaller, YamlMarshaller
from deepstack.telemetry import pipeline_running
from deepstack.utils import is_in_jupyter
...
class Pipeline:
    ...
    def __init__(
        ...
        debug_path: Union[Path, str] = Path(".deepstack_debug/"),
    ):
        """
    ...
    def from_dict(
        ...
        debug_path = Path(data.get("debug_path", ".deepstack_debug/"))
    ...
    def add_component(self, name: str, instance: Component) -> None:
        ...
        if getattr(instance, "__deepstack_added_to_pipeline__", None):
            msg = (
        ...
        setattr(instance, "__deepstack_added_to_pipeline__", self)
        ...
        self.graph.add_node(
            name,
            instance=instance,
            input_sockets=instance.__deepstack_input__._sockets_dict,  # type: ignore[attr-defined]
            output_sockets=instance.__deepstack_output__._sockets_dict,  # type: ignore[attr-defined]
            visits=0,
        )
    ...
    def _validate_input(self, data: Dict[str, Any]):
        ...
        * Each Component doesn't receive inputs that are already sent by another Component
        ...
        for component_name, component_inputs in data.items():
            ...
            for socket_name, socket in instance.__deepstack_input__._sockets_dict.items():
            ...
            for input_name in component_inputs.keys():
                if input_name not in instance.__deepstack_input__._sockets_dict:
    ...
    def run(  # noqa: C901, PLR0912, PLR0915 pylint: disable=too-many-branches
        ...
        # defined in `deepstack/pipeline.py`.
        ...
        for component_name, component_inputs in data.items():
            ...
            for component_input, input_value in component_inputs.items():
                ...
                if instance.__deepstack_input__._sockets_dict[component_input].is_variadic:
        ...
        for node_name in self.graph.nodes:
            ...
            if len(component.__deepstack_input__._sockets_dict) == 0:
                # Component has no input, can run right away
                to_run.append((node_name, component))
                continue

            for socket in component.__deepstack_input__._sockets_dict.values():
        ...
        with tracing.tracer.trace(
            "deepstack.pipeline.run",
            tags={
                "deepstack.pipeline.input_data": data,
                "deepstack.pipeline.output_data": final_outputs,
                "deepstack.pipeline.debug": debug,
                "deepstack.pipeline.metadata": self.metadata,
                "deepstack.pipeline.max_loops_allowed": self.max_loops_allowed,
            },
        ):
            ...
            while len(to_run) > 0:
                ...
                if any(socket.is_variadic for socket in comp.__deepstack_input__._sockets_dict.values()) and not getattr(  # type: ignore
                    comp, "is_greedy", False
                ):
                    there_are_non_variadics = False
                    for _, other_comp in to_run:
                        if not any(socket.is_variadic for socket in other_comp.__deepstack_input__._sockets_dict.values()):  # type: ignore
                ...
                if name in last_inputs and len(comp.__deepstack_input__._sockets_dict) == len(last_inputs[name]):  # type: ignore
                    ...
                    with tracing.tracer.trace(
                        "deepstack.component.run",
                        tags={
                            "deepstack.component.name": name,
                            "deepstack.component.type": comp.__class__.__name__,
                            "deepstack.component.input_types": {
                                k: type(v).__name__ for k, v in last_inputs[name].items()
                            },
                            "deepstack.component.input_spec": {
                                key: {
                                    "type": value.type.__name__ if isinstance(value.type, type) else str(value.type),
                                    "senders": value.senders,
                                }
                                for key, value in comp.__deepstack_input__._sockets_dict.items()  # type: ignore
                            },
                            "deepstack.component.output_spec": {
                                key: {
                                    "type": value.type.__name__ if isinstance(value.type, type) else str(value.type),
                                    "senders": value.receivers,
                                }
                                for key, value in comp.__deepstack_output__._sockets_dict.items()  # type: ignore
                            },
                        },
                    ) as span:
                        span.set_content_tag("deepstack.component.input", last_inputs[name])
                        ...
                        span.set_tags(tags={"deepstack.component.visits": self.graph.nodes[name]["visits"]})
                        span.set_content_tag("deepstack.component.output", res)
                    ...
                    for sender_component_name, receiver_component_name, edge_data in self.graph.edges(data=True):
                        ...
                        is_greedy = pair[1].__deepstack_is_greedy__
                ...
                if len(to_run) == 0 and len(waiting_for_input) > 0:
                    ...
                    if (
                        ...
                        for name, comp in waiting_for_input:
                            is_variadic = any(socket.is_variadic for socket in comp.__deepstack_input__._sockets_dict.values())  # type: ignore
                            has_only_defaults = all(
                                not socket.is_mandatory for socket in comp.__deepstack_input__._sockets_dict.values()  # type: ignore
                            )
                            if is_variadic and not comp.__deepstack_is_greedy__ or has_only_defaults:  # type: ignore[attr-defined]
                                break
                        else:
                        ...
                        for input_socket in comp.__deepstack_input__._sockets_dict.values():  # type: ignore
                    ...
                    for name, comp in waiting_for_input:
                        ...
                        is_variadic = any(socket.is_variadic for socket in comp.__deepstack_input__._sockets_dict.values())  # type: ignore
                        if is_variadic and not comp.__deepstack_is_greedy__:  # type: ignore[attr-defined]
                            ...
                            for other_name, other_comp in waiting_for_input:
                                ...
                                there_are_only_lazy_variadics &= (
                                    any(
                                        socket.is_variadic for socket in other_comp.__deepstack_input__._sockets_dict.values()  # type: ignore
                                    )
                                    and not other_comp.__deepstack_is_greedy__  # type: ignore[attr-defined]
                                )
                        ...
                        if has_only_defaults:
                            ...
                            for other_name, other_comp in waiting_for_input:
                                ...
                                there_are_only_components_with_defaults &= all(
                                    not s.is_mandatory for s in other_comp.__deepstack_input__._sockets_dict.values()  # type: ignore
                                )
