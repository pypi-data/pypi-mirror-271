...
DEEPSTACK_LOGGING_USE_JSON_ENV_VAR = "DEEPSTACK_LOGGING_USE_JSON"
DEEPSTACK_LOGGING_IGNORE_STRUCTLOG_ENV_VAR = "DEEPSTACK_LOGGING_IGNORE_STRUCTLOG"
...
def _patch_structlog_call_information(logger: logging.Logger) -> None:
    ...
    # We need to patch their patch to hide `deepstack.logging` from the traceback.
    try:
        ...
        # completely copied from structlog. We only add `deepstack.logging` to the list of ignored frames
        # pylint: disable=unused-variable
        def findCaller(stack_info: bool = False, stacklevel: int = 1) -> typing.Tuple[str, int, str, Optional[str]]:
            try:
                sinfo: Optional[str]
                # we need to exclude `deepstack.logging` from the stack
                f, name = _find_first_app_frame_and_name(["logging", "deepstack.logging"])
...
def getLogger(name: str) -> PatchedLogger:
    """
    Get the Deepstack logger, a patched version of the one from the standard library.
...
def correlate_logs_with_traces(_: "WrappedLogger", __: str, event_dict: "EventDict") -> "EventDict":
    ...
    """
    import deepstack.tracing.tracer  # to avoid circular imports

    if not deepstack.tracing.is_tracing_enabled():
        return event_dict

    current_span = deepstack.tracing.tracer.current_span()
    if current_span:
...
def configure_logging(use_json: Optional[bool] = None) -> None:
    """
    Configure logging for Deepstack.
    ...
    - If `structlog` is installed, we configure it to format log entries including its key-value data. To disable this
      behavior set the environment variable `DEEPSTACK_LOGGING_IGNORE_STRUCTLOG` to `true`.
    - If `structlog` is installed, you can JSON format all logs. Enable this by
        ...
        - setting the environment variable `DEEPSTACK_LOGGING_USE_JSON` to `true`
    """
    import deepstack.utils.jupyter  # to avoid circular imports
    ...
    if os.getenv(DEEPSTACK_LOGGING_IGNORE_STRUCTLOG_ENV_VAR, "false").lower() == "true":
    ...
    if use_json is None:  # explicit parameter takes precedence over everything else
        use_json_env_var = os.getenv(DEEPSTACK_LOGGING_USE_JSON_ENV_VAR)
        if use_json_env_var is None:
            # We try to guess if we are in an interactive terminal or not
            interactive_terminal = (
                sys.stderr.isatty() or hasattr(builtins, "__IPYTHON__") or deepstack.utils.jupyter.is_in_jupyter()
            )
    ...
    structlog.configure(
        ...
        logger_factory=structlog.stdlib.LoggerFactory(ignore_frame_names=["deepstack.logging"]),
    ...
    handler.name = "DeepstackLoggingHandler"
    ...
    old_handlers = [
        ...
        if not (isinstance(h, logging.StreamHandler) and h.name == "DeepstackLoggingHandler")
    ]
