...
from deepstack.core.serialization import generate_qualified_class_name
from deepstack.telemetry._environment import collect_system_specs

if TYPE_CHECKING:
    from deepstack.core.pipeline import Pipeline


DEEPSTACK_TELEMETRY_ENABLED = "DEEPSTACK_TELEMETRY_ENABLED"
CONFIG_PATH = Path("~/.deepstack/config.yaml").expanduser()
...
class Telemetry:
    """
    Deepstack reports anonymous usage statistics to support continuous software improvements for all its users.

    You can opt-out of sharing usage statistics by manually setting the environment
    variable `DEEPSTACK_TELEMETRY_ENABLED` as described for different operating systems on the
    [documentation page](https://docs.deepstack.khulnasoft.com/docs/telemetry#how-can-i-opt-out).

    Check out the documentation for more details: [Telemetry](https://docs.deepstack.khulnasoft.com/docs/telemetry).
    """
    ...
    def __init__(self):
        ...
        if CONFIG_PATH.exists():
            ...
            logger.info(
                "Deepstack sends anonymous usage data to understand the actual usage and steer dev efforts "
                "towards features that are most meaningful to users. You can opt-out at anytime by manually "
                "setting the environment variable DEEPSTACK_TELEMETRY_ENABLED as described for different "
                "operating systems in the [documentation page](https://docs.deepstack.khulnasoft.com/docs/telemetry#how-can-i-opt-out). "
                "More information at [Telemetry](https://docs.deepstack.khulnasoft.com/docs/telemetry)."
            )
...
telemetry = None
if os.getenv("DEEPSTACK_TELEMETRY_ENABLED", "true").lower() in ("true", "1"):
    telemetry = Telemetry()
