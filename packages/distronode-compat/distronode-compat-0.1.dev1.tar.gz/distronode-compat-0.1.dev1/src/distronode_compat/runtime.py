"""Distronode runtime environment manager."""
...
from distronode_compat.config import (
    DistronodeConfig,
    distronode_collections_path,
    distronode_version,
    parse_distronode_version,
)
from distronode_compat.constants import (
    META_MAIN,
    MSG_INVALID_FQRL,
    RC_DISTRONODE_OPTIONS_ERROR,
    REQUIREMENT_LOCATIONS,
)
from distronode_compat.errors import (
    DistronodeCommandError,
    DistronodeCompatError,
    InvalidPrerequisiteError,
    MissingDistronodeError,
)
from distronode_compat.loaders import colpath_from_path, yaml_from_file
...
class DistronodeWarning(Warning):
    """Warnings related to Distronode runtime."""


@dataclass
class Collection:
    """Container for Distronode collection information."""
...
class Plugins:  # pylint: disable=too-many-instance-attributes
    """Dataclass to access installed Distronode plugins, uses distronode-doc to retrieve them."""
    ...
    def __getattribute__(self, attr: str):  # noqa: ANN204
        ...
        if attr in {
            ...
            try:
                ...
                if distronode_version() < Version("2.14") and attr in {"filter", "test"}:
                    msg = "Distronode version below 2.14 does not support retrieving filter and test plugins."
                    raise RuntimeError(msg) from exc
                proc = self.runtime.run(
                    ["distronode-doc", "--json", "-l", "-t", attr],
                )
                data = json.loads(proc.stdout)
                if not isinstance(data, dict):  # pragma: no cover
                    msg = "Unexpected output from distronode-doc"
                    raise DistronodeCompatError(msg) from exc
                result = data
        else:
            result = super().__getattribute__(attr)
...
class Runtime:
    """Distronode Runtime manager."""

    _version: Version | None = None
    collections: OrderedDict[str, Collection] = OrderedDict()
    cache_dir: Path | None = None
    # Used to track if we have already initialized the Distronode runtime as attempts
    # to do it multiple tilmes will cause runtime warnings from within distronode-core
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize Distronode runtime environment.

        :param project_dir: The directory containing the Distronode project. If
                            not mentioned it will be guessed from the current
                            working directory.
        :param isolated: Assure that installation of collections or roles
                         does not affect Distronode installation, an unique cache
                         directory being used instead.
        :param min_required_version: Minimal version of Distronode required. If
                                     not found, a :class:`RuntimeError`
                                     exception is raised.
        :param require_module: If set, instantiation will fail if Distronode
                               Python module is missing or is not matching
                               the same version as the Distronode command line.
                               That is useful for consumers that expect to
                               also perform Python imports from Distronode.
        ...
        if not self.version_in_range(lower=min_required_version):
            msg = f"Found incompatible version of distronode runtime {self.version}, instead of {min_required_version} or newer."
        ...
        def warning(
            ...
            *,
            ...
            """Override distronode.utils.display.Display.warning to avoid printing warnings."""
            warnings.warn(
                message=msg,
                category=DistronodeWarning,
                stacklevel=2,
                source={"msg": msg},
            )

        # Monkey patch distronode warning in order to use warnings module.
        ...
        if self.config.collections_scan_sys_path:
            for path in sys.path:
                if (
                    ...
                    and (Path(path) / "distronode_collections").is_dir()
                ):
    ...
    def load_collections(self) -> None:
        ...
        proc = self.run(["distronode-galaxy", "collection", "list", "--format=json"])
        if proc.returncode == RC_DISTRONODE_OPTIONS_ERROR and (
            no_collections_msg in proc.stdout or no_collections_msg in proc.stderr
        ):
            _logger.debug("Distronode reported no installed collections at all.")
            return
    ...
    def _ensure_module_available(self) -> None:
        """Assure that Distronode Python module is installed and matching CLI version."""
        distronode_release_module = None
        with contextlib.suppress(ModuleNotFoundError, ImportError):
            distronode_release_module = importlib.import_module("distronode.release")

        if distronode_release_module is None:
            msg = "Unable to find Distronode python module."
            raise RuntimeError(msg)

        distronode_module_version = Version(
            distronode_release_module.__version__,
        )
        if distronode_module_version != self.version:
            msg = f"Distronode CLI ({self.version}) and python module ({distronode_module_version}) versions do not match. This indicates a broken execution environment."
            raise RuntimeError(msg)

        # For distronode 2.15+ we need to initialize the plugin loader
        # https://github.com/distronode/distronode-lint/issues/2945
        if not Runtime.initialized:
            ...
            if self.version >= Version("2.15.0.dev0"):
                ...
                from distronode.utils.collection_loader._collection_finder import (  # pylint: disable=import-outside-toplevel
                ...
                col_path += os.path.dirname(  # noqa: PTH120
                    os.environ.get(distronode_collections_path(), "."),
                ).split(":")
                _DistronodeCollectionFinder(  # noqa: SLF001
                    paths=col_path,
    ...
    def run(  # ruff: disable=PLR0913
        ...
        *,
        ...
        """Execute a command inside an Distronode environment.
        ...
        # Presence of distronode debug variable or config option will prevent us
        # from parsing its JSON output due to extra debug messages on stdout.
        env["DISTRONODE_DEBUG"] = "0"

        # https://github.com/distronode/distronode-lint/issues/3522
        env["DISTRONODE_VERBOSE_TO_STDERR"] = "True"
    ...
    def version(self) -> Version:
        ...
        proc = self.run(["distronode", "--version"])
        if proc.returncode == 0:
            self._version = parse_distronode_version(proc.stdout)
            return self._version

        msg = "Unable to find a working copy of distronode executable."
        raise MissingDistronodeError(msg, proc=proc)
    ...
    def version_in_range(
        ...
        """Check if Distronode version is inside a required range.
    ...
    def install_collection(
        ...
        *,
        ...
        """Install an Distronode collection.

        Can accept arguments like:
            'foo.bar:>=1.2.3'
            'git+https://github.com/distronode-collections/distronode.posix.git,main'
        """
        cmd = [
            "distronode-galaxy",
            "collection",
            "install",
            "-vvv",  # this is needed to make distronode display important info in case of failures
        ]
        if force:
            cmd.append("--force")
        ...
        # As distronode-galaxy install is not able to automatically determine
        ...
        if destination and str(destination) not in cpaths:
            ...
            # we hack distronode_collections_path instead and inject our own path there.
        ...
        run = self.run(
            cmd,
            retry=True,
            env={**self.environ, distronode_collections_path(): ":".join(cpaths)},
        )
        if run.returncode != 0:
    ...
    def install_collection_from_disk(
        ...
        if not self.version_in_range(upper="2.11"):
            ...
            return
        # older versions of distronode able unable to install without building
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if run.returncode != 0:
                ...
                raise DistronodeCommandError(run)
    ...
    def install_requirements(  # noqa: C901
        ...
        *,
        ...
        if not isinstance(reqs_yaml, (dict, list)):
            msg = f"{requirement} file is not a valid Distronode requirements file."
            raise InvalidPrerequisiteError(msg)

        if isinstance(reqs_yaml, dict):
            for key in reqs_yaml:
                if key not in ("roles", "collections"):
                    msg = f"{requirement} file is not a valid Distronode requirements file. Only 'roles' and 'collections' keys are allowed at root level. Recognized valid locations are: {', '.join(REQUIREMENT_LOCATIONS)}"
                    raise InvalidPrerequisiteError(msg)
        ...
        if isinstance(reqs_yaml, list) or "roles" in reqs_yaml:
            ...
            if offline:
                ...
                if result.returncode != 0:
                    ...
                    raise DistronodeCommandError(result)
        ...
        if "collections" in reqs_yaml and reqs_yaml["collections"] is not None:
            ...
            for collection in reqs_yaml["collections"]:
                if isinstance(collection, dict) and collection.get("type", "") == "git":
                    _logger.info(
                        "Adding '--pre' to distronode-galaxy collection install because we detected one collection being sourced from git.",
                    )
                    cmd.append("--pre")
                    break
            if offline:
                ...
                result = self.run(
                    cmd,
                    retry=retry,
                    env={**os.environ, "DISTRONODE_COLLECTIONS_PATH": ":".join(cpaths)},
                )
                if result.returncode != 0:
                    _logger.error(result.stdout)
    ...
    def prepare_environment(  # noqa: C901
        ...
        *,
        ...
        # https://docs.distronode.com/distronode-tower/latest/html/userguide/projects.html#distronode-galaxy-support
        # https://docs.distronode.com/distronode-tower/latest/html/userguide/projects.html#collections-support
        for req_file in REQUIREMENT_LOCATIONS:
            self.install_requirements(Path(req_file), retry=retry, offline=offline)

        self._prepare_distronode_paths()
        ...
        if Path("galaxy.yml").exists():
            if destination:
                ...
                colpath = Path(
                    f"{destination}/distronode_collections/{colpath_from_path(Path.cwd())}",
                )
    ...
    def require_collection(
        ...
        *,
        ...
        if not paths or not isinstance(paths, list):
            msg = f"Unable to determine distronode collection paths. ({paths})"
            raise InvalidPrerequisiteError(
                msg,
            )
        ...
        for path in paths:
            collpath = Path(path) / "distronode_collections" / ns / coll
            if collpath.exists():
    ...
    def _prepare_distronode_paths(self) -> None:
        """Configure Distronode environment variables."""
        try:
            ...
            msg = "Unexpected distronode configuration"
            raise RuntimeError(msg) from exc
        ...
        if library_paths != self.config.DEFAULT_MODULE_PATH:
            self._update_env("DISTRONODE_LIBRARY", library_paths)
        if collections_path != self.config.collections_paths:
            self._update_env(distronode_collections_path(), collections_path)
        if roles_path != self.config.default_roles_path:
            self._update_env("DISTRONODE_ROLES_PATH", roles_path)
    ...
    def _install_galaxy_role(
        ...
        *,
        ...
        Our implementation aims to match distronode-galaxy's behaviour for installing
        roles from a tarball or scm. For example distronode-galaxy will install a role
        ...
        _logger.info(
            "Using %s symlink to current repository in order to enable Distronode to find the role using its expected full name.",
            link_path,
        )
...
def _get_role_fqrn(galaxy_infos: dict[str, Any], project_dir: Path) -> str:
    ...
    if len(role_name) == 0:
        ...
        role_name = re.sub(r"(distronode-|distronode-role-)", "", role_name).split(
            ".",
            maxsplit=2,
        )[-1]
...
def _get_galaxy_role_ns(galaxy_infos: dict[str, Any]) -> str:
    ...
    if not isinstance(role_namespace, str):
        ...
        raise DistronodeCompatError(msg)
...
def search_galaxy_paths(search_dir: Path) -> list[str]:
    ...
    for file in [".", *os.listdir(search_dir)]:
        ...
        # distronode galaxy does at this moment.
