...
from distronode_compat.constants import INVALID_PREREQUISITES_RC
from distronode_compat.errors import (
    DistronodeCommandError,
    DistronodeCompatError,
    InvalidPrerequisiteError,
)
from distronode_compat.runtime import (
    CompletedProcess,
    Runtime,
    is_url,
    search_galaxy_paths,
...
def test_runtime_version_outdated(require_module: bool) -> None:
    ...
    with pytest.raises(RuntimeError, match="Found incompatible version of distronode"):
        Runtime(min_required_version="9999.9.9", require_module=require_module)


def test_runtime_missing_distronode_module(monkeypatch: MonkeyPatch) -> None:
    """Checks that we produce a RuntimeError when distronode module is missing."""
    ...
    with pytest.raises(RuntimeError, match="Unable to find Distronode python module."):
        Runtime(require_module=True)


def test_runtime_mismatch_distronode_module(monkeypatch: MonkeyPatch) -> None:
    """Test that missing module is detected."""
    monkeypatch.setattr("distronode.release.__version__", "0.0.0", raising=False)
...
def test_runtime_require_module() -> None:
    ...
    from distronode.utils.collection_loader import (  # pylint: disable=import-outside-toplevel
        DistronodeCollectionConfig,
    )

    DistronodeCollectionConfig.playbook_paths = "."
    # Calling it again in order to see that it does not produce UserWarning: DistronodeCollectionFinder has already been configured
    # which is done by Distronode core 2.15+. We added special code inside Runtime
...
def test_runtime_version_fail_module(mocker: MockerFixture) -> None:
    """Tests for failure to detect Distronode version."""
    patched = mocker.patch(
        "distronode_compat.runtime.parse_distronode_version",
        autospec=True,
    )
    patched.side_effect = InvalidPrerequisiteError(
        "Unable to parse distronode cli version",
    )
    runtime = Runtime()
    with pytest.raises(
        InvalidPrerequisiteError,
...
def test_runtime_version_fail_cli(mocker: MockerFixture) -> None:
    """Tests for failure to detect Distronode version."""
    mocker.patch(
        "distronode_compat.runtime.Runtime.run",
...
def test_runtime_prepare_distronode_paths_validation() -> None:
    """Check that we validate collection_path."""
    runtime = Runtime()
    runtime.config.collections_paths = "invalid-value"  # type: ignore[assignment]
    with pytest.raises(RuntimeError, match="Unexpected distronode configuration"):
        runtime._prepare_distronode_paths()


@pytest.mark.parametrize(
    ("folder", "role_name", "isolated"),
    (
        ("distronode-role-sample", "acme.sample", True),
...
def test_runtime_install_role(
    ...
    result = runtime.run(["distronode-galaxy", "list"])
...
@pytest.mark.parametrize(
    ...
    (
        (
            Path("/dev/null"),
            InvalidPrerequisiteError,
            "file is not a valid Distronode requirements file",
        ),
        (
            Path(__file__).parent / "assets" / "requirements-invalid-collection.yml",
            DistronodeCommandError,
            "Got 1 exit code while running: distronode-galaxy",
        ),
        (
...
def test_prerun_reqs_v1(caplog: pytest.LogCaptureFixture, runtime: Runtime) -> None:
    ...
    assert any(
        msg.startswith("Running distronode-galaxy role install") for msg in caplog.messages
    )
    assert all(
        "Running distronode-galaxy collection install" not in msg
...
def test_prerun_reqs_v2(caplog: pytest.LogCaptureFixture, runtime: Runtime) -> None:
    ...
    with cwd(path):
        with caplog.at_level(logging.INFO):
            runtime.prepare_environment()
        assert any(
            msg.startswith("Running distronode-galaxy role install")
            for msg in caplog.messages
        )
        assert any(
            msg.startswith("Running distronode-galaxy collection install")
            for msg in caplog.messages
        )
...
def test_require_collection_invalid_collections_path(runtime: Runtime) -> None:
    ...
    with pytest.raises(
        InvalidPrerequisiteError,
        match="Unable to determine distronode collection paths",
    ):
...
def test_require_collection_preexisting_broken(tmp_path: pathlib.Path) -> None:
    ...
    dest = pathlib.Path(dest_path) / "distronode_collections" / "foo" / "bar"
...
def test_require_collection_missing(
    ...
    with pytest.raises(DistronodeCompatError) as pytest_wrapped_e:
...
def test_install_collection_git(runtime: Runtime) -> None:
    ...
    runtime.install_collection(
        "git+https://github.com/distronode-collections/distronode.posix,main",
    )
...
def test_install_collection_dest(runtime: Runtime, tmp_path: pathlib.Path) -> None:
    ...
    # Since Distronode 2.15.3 there is no guarantee that this will install the collection at requested path
...
""",
    ...
    with pytest.raises(DistronodeCompatError, match="Role namespace must be string, not"):
        runtime_tmp._install_galaxy_role(runtime_tmp.project_dir, role_name_check=1)
...
""",
    ...
    runtime_tmp._install_galaxy_role(runtime_tmp.project_dir, role_name_check=2)
    result = runtime_tmp.run(["distronode-galaxy", "list"])
...
def test_upgrade_collection(runtime_tmp: Runtime) -> None:
    ...
    # ensure that we inject our tmp folders in distronode paths
    runtime_tmp.prepare_environment()
...
def test_runtime_env_distronode_library(monkeypatch: MonkeyPatch) -> None:
    """Verify that custom path specified using DISTRONODE_LIBRARY is not lost."""
    path_name = "foo"
    monkeypatch.setenv("DISTRONODE_LIBRARY", path_name)
...
@pytest.mark.parametrize(
    ...
    (
        pytest.param(
            ...
            "default",
            [
                "distronode.posix",  # from tests/requirements.yml
                "distronode.utils",  # from galaxy.yml
...
def test_install_collection_from_disk(
    ...
    rmtree(
        pathlib.Path(
            "~/.distronode/collections/distronode_collections/acme/goodies",
        ).expanduser(),
        ignore_errors=True,
    )
    with cwd(Path(path)):
        ...
        result = runtime.run(["distronode-playbook", f"molecule/{scenario}/converge.yml"])
...
def test_install_collection_from_disk_fail() -> None:
    ...
    with cwd(Path("test/collections/acme.broken")):
        ...
        with pytest.raises(RuntimeError) as exc_info:
            runtime.prepare_environment(install_local=True)
        # based on version of Distronode used, we might get a different error,
        ...
        assert exc_info.match(
            "(is missing the following mandatory|Got 1 exit code while running: distronode-galaxy collection build)",
        )
...
def test_runtime_plugins(runtime: Runtime) -> None:
    ...
    # distronode.netcommon.restconf might be in httpapi
    assert isinstance(runtime.plugins.httpapi, dict)
    # "distronode.netcommon.default" might be in runtime.plugins.netconf
    ...
    if distronode_version() < Version("2.14.0"):
        ...
        # distronode-doc below 2.14 does not support listing 'test' and 'filter' types:
        with pytest.raises(RuntimeError):
            assert "is_abs" in runtime.plugins.test
        with pytest.raises(RuntimeError):
            assert "bool" in runtime.plugins.filter
    else:
        assert "distronode.builtin.sudo" in runtime.plugins.become
        assert "distronode.builtin.memory" in runtime.plugins.cache
        assert "distronode.builtin.default" in runtime.plugins.callback
        assert "distronode.builtin.local" in runtime.plugins.connection
        assert "distronode.builtin.ini" in runtime.plugins.inventory
        assert "distronode.builtin.env" in runtime.plugins.lookup
        assert "distronode.builtin.sh" in runtime.plugins.shell
        assert "distronode.builtin.host_group_vars" in runtime.plugins.vars
        assert "distronode.builtin.file" in runtime.plugins.module
        assert "distronode.builtin.free" in runtime.plugins.strategy
        assert "distronode.builtin.is_abs" in runtime.plugins.test
        assert "distronode.builtin.bool" in runtime.plugins.filter
