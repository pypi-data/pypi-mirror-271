...
class EvaluationWizard(Wizard):
    ...
    def add_accuracy(self, role: str = 'total', drawable=True):
        return self.add_evaluation_handler_type('Accuracy', lambda: Accuracy(drawable))

    def add_confusion_matrix(self, role: str = 'total', drawable=True):
        return self.add_evaluation_handler_type('ConfusionMatrix', lambda: ConfusionMatrix(drawable))
