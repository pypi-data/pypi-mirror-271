...
def get_or_default_alembic_config() -> Config:
    ...
    logger.warning(
        f'Could not find alembic config in {cwd}, please run `alembic init` or ensure you are in the root folder',
    )
    return Config()
...
def get_or_raise_alembic_config() -> Config:
    ...
    if alembic_config is None or alembic_config.config_file_name is None:
        raise RuntimeError("Please run the 'db init' command first!")
