...
def decimate(s, r, n=None, fir=False):
    ...
    if fir:
        if n is None:
            n = 30
        b = scisig.firwin(n, 1.0/r)
        a = 1
        f = scisig.lfilter(b, a, s)
    else: #iir
        if n is None:
            n = 8
        b, a = scisig.cheby1(n, 0.05, 0.8/r)
        f = scisig.filtfilt(b, a, s)
...
def interp(s, r, l=4, alpha=0.5):
    ...
    alpha is 0.5.
    """
    b = scisig.firwin(2*l*r+1, alpha/r);
    a = 1
    return r*scisig.lfilter(b, a, upsample(s, r))[r*l+1:-1]
...
def resample(s, p, q, h=None):
    ...
    if h is None: #design filter
        ...
        beta = scisig.kaiser_beta(rejection_db)
...
def upfirdn(s, h, p, q):
    ...
    """
    return downsample(scisig.fftconvolve(h, upsample(s, p)), q)
