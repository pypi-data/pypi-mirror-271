...
class ZNLE14ChannelTrace(InstrumentChannel):
    ...
    def get_freq_setpoints(self):
        return self.root_instrument.params.get_freq_setpoints(self._parent._channel_num)
...
class ZNLEParams(InstrumentChannel):
    ...
    def get_freq_setpoints(self, channel_num: int):
        """Get frequency setpoints."""
        start = self.freq_start.get(channel_num)
        stop = self.freq_stop.get(channel_num)
        num_points = self.sweep_points.get(channel_num)
        sweep_type = self.sweep_type.get(channel_num).strip()
