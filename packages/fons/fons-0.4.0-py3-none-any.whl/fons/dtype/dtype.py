...
class DType(Routine):
    ...
    id_attrs = []
    set_id_attrs = (
        False  # if True, `for a in self.id_attrs: setattr(self, a, id[a])` is called
    )
    time_key = None
    time_freq = None
    storable = True
        ...
        *,
        ...
        for attr in self.id_attrs:
            try:
                ...
                if (
                    clsv is not None
                    and not inspect.isfunction(clsv)
                    and not inspect.ismethod(clsv)
                ):
                    value = getattr(self.__class__, attr)
                else:
                    raise ValueError(
                        "Got id: {}, which is missing some params from: {}".format(
                            id, self.id_attrs
                        )
                    )
            self.id[attr] = value
        ...
        if load is True:
            load = {}
        elif load is None or load is False:
            pass
        elif not isinstance(load, dict):
            load = {"tp": load}
        ...
        if reduce is True:
            reduce = {}
        elif reduce is None or reduce is False:
            pass
        elif not isinstance(reduce, dict):
            reduce = {"keep_periods": reduce}
        ...
        _freq = self.save_freq[
            next(i for i, x in enumerate(self.save_freq) if x.isalpha()) :
        ]
        f_extra = {
            "save": {
                ...
                "sync": merging.SYNCS[_freq]
                if _freq in merging.SYNCS
                else "random-0.4-0.8",
                "lock": "next",
            },
        }
        ...
        for func_name, inp in zip(("update", "save"), (update, save)):
            ...
            if isinstance(inp, bool):
                ...
                inp,
                (int, float, td, str, pd.offsets.DateOffset)
                or hasattr(inp, "__call__"),
            ):
                temp["interval"] = inp
            elif isinstance(inp, dict):
                if inp.get("args"):
                    raise ValueError(
                        "Passing args to '{}' not allowed; got: {}".format(
                            func_name, inp["args"]
                        )
                    )
                if "kwargs" in inp:
                    ...
                    inp["kwargs"] = dict(
                        params.get("kwargs", {}),
                        **(inp["kwargs"] if inp["kwargs"] is not None else {})
                    )
                temp.update(inp)
    ...
    def load(self, tp, extend=False, **kw):
        ...
        if kw.get("TEST_MODE") is None:
            kw["TEST_MODE"] = self.TEST_MODE
    ...
    def _resolve_tp(self, tp):
        ...
        if hasattr(tp, "__call__") and not isinstance(tp, pd.DateOffset):
            tp = tp()
        return tp
    ...
    def _relay_create_merger(self):
        ...
        if not merge:
            return
        elif self.conns.get("merge") is None:
            return
        freqs = (
            merge
            if isinstance(merge, (list, tuple))
                ...
                else store._get_higher_freqs(self.save_freq)
            )
        )
        if not freqs:
            return
        ...
        self.conns["merge"].send(
            {
                ...
                "data": {
                    "id": self.id_str,
                    "cls": self.merger_cls
                    if self.merger_cls is not None
                    else self.__class__,
                    "args": (self.id,),
    ...
    def _relay_start_merger(self):
        if not self._ui["merger_relay_created"]:
            return
        logger.debug("{} - relaying start merger.".format(self.id_str))
        self.conns["merge"].send({"method": "start", "data": self.id_str})
        self._ui["merger_relay_started"] = dt.utcnow()

    def _relay_stop_merger(self):
        if not self._ui["merger_relay_started"]:
            return
        logger.debug("{}: relaying stop merger".format(self.id_str))
        self.conns["merge"].send({"method": "stop", "data": self.id_str})
