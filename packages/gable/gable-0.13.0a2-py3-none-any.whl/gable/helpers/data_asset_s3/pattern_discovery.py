...
def _list_files(
    ...
    try:
        ...
        log_debug("Listed directories to crawl: {}", dirpaths)
        files = []
...
def _trim_recent_patterns(
    ...
    for suffix in suffixes:
        ...
        if len(include) > 0 and any(
            ...
            if re.match(f".*/({YEAR_REGEX})/({MONTH_REGEX})/?$", prefix) and re.match(
                DAY_REGEX, suffix
            ):
                max_num_is_day = True
                ...
                for reg, format in [
                    ...
                    if re.match(reg, suffix):
                        ...
                        try:
                            ...
                            log_error(
                                "Failed to parse date with {}: {}", format, str(e)
                            )
