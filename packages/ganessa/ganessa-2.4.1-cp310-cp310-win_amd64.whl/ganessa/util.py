...
def get_this_python_display_name_company(company=None):
    """Get DisplayName from current python interpreter if registered"""
    default_company = "PythonCore"
    if company is None:
        company = default_company
    try:
        registry_key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, f"SOFTWARE\\Python\\{company}")
        version_number = 0
        while True:
            try:
...
def get_this_python_display_name():
    ...
    try:
        registry_key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, "SOFTWARE\\Python")
        company_number = 0
        while True:
            try:
            ...
            if company.lower() == "pylauncher":
                continue
            # print("\nExamining:", company)
            if (retval := get_this_python_display_name_company(company)):
                return retval
    except WindowsError:
        pass
    return ()
