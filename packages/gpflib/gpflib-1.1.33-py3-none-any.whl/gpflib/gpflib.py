    ...
    digraph Grid_modules{
     ...
     '''
        if self.GetUnitKV(Unit,"Word") == 0:
            return
        Script=Heade+"\n"
        Script+=self.UnitInfo(Unit)+'[shape="record",style="filled",fillcolor="Green",label=" '+Unit+self.GetUnitKV(Unit,"Word")+' "]'+"\n"
        Atts=self.GetUnitKV(Unit)
        for K,Vs in Atts.items():
            if (K[0] == "U" or K[0] == "R") and len(K) > 5:
                continue    
            V=" ".join(Vs)            
            Script+=self.UnitInfo(K)+'[style="filled", fillcolor="gray",label=" '+V+' "]'+"\n"
