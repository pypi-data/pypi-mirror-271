...
def baseflow_separation(streamflow, bt, qft, filter_parameter, passes):
    ...
    only: quantile1d_r
    """
    _libfcore.f90wrap_baseflow_separation(streamflow=streamflow, bt=bt, qft=qft, \
        filter_parameter=filter_parameter, passes=passes)
...
def rc(p, q):
    ...
    """
    res = _libfcore.f90wrap_rc(p=p, q=q)
    return res
...
def rchf(p, q):
    ...
    """
    res = _libfcore.f90wrap_rchf(p=p, q=q)
    return res
...
def rclf(p, q):
    ...
    """
    res = _libfcore.f90wrap_rclf(p=p, q=q)
    return res
...
def rch2r(p, q):
    ...
    it returns the result of RCLF computation
    """
    res = _libfcore.f90wrap_rch2r(p=p, q=q)
    return res
...
def cfp(q, quant):
    ...
    """
    res = _libfcore.f90wrap_cfp(q=q, quant=quant)
    return res
...
def elt(p, q):
    ...
    """
    res = _libfcore.f90wrap_elt(p=p, q=q)
    return res
