...
class MeshDT(f90wrap.runtime.FortranDerivedType):
    ...
    def __init__(self, setup, nrow, ncol, npar, ng, handle=None):
        ...
        result = _libfcore.f90wrap_meshdt_initialise(setup=setup._handle, nrow=nrow, \
            ncol=ncol, npar=npar, ng=ng)
    ...
    def __del__(self):
        ...
        try:
            if self._alloc:
                _libfcore.f90wrap_meshdt_finalise(this=self._handle)
        except:
            pass
...
def meshdt_copy(self):
    ...
    """
    this_copy = _libfcore.f90wrap_meshdt_copy(this=self._handle)
    this_copy = \
