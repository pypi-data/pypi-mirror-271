...
#   if path contains mb_izpay, split the path and get the first part
    ...
    # change the flavor in the app
    # get env file data
    flavor, bundleId, appName, appIconPath, primaryColor = getEnv(flavor, root, project="izpay", console=console)
    ...
    with console.status(
        ...
        changeSplash(root, appIconPath.replace("/", os.sep),primaryColor, project="izpay")
        status.update("Updating onesignal icons")
...
@app.command()
def setup_remote_config(path: str = Option(
        os.getcwd,
        "--path",
        "-p",
        help="Path to the project",
    ...
    if "mb_izpay" not in path:
        raise ValueError("This command must be run from a loyalty flutter app")
    setupRemoteConfig(path, console)
