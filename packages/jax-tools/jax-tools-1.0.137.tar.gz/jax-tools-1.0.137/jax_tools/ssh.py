...
class SSHClient(object):
    ...
    def run_cmd(self, cmd: str, read_line: bool = False, time_out: int = 300) -> Optional[str]:
        ...
        try:
            ...
            logger.error('Connection exception when run cmd, User login info may be wrong, or connection has been closed, '
                         ' msg: %s' % e)
        return result

    def run_cmd_safety(self, cmd: str, read_line: bool = False, time_out: int = 300) -> Optional[str]:
        """
        Run command on remote host
        Args:
            cmd (str): command to run
