...
class HelperNew:
    ...
    def _find_missing_error_indices(self, d: Series | DataFrame) -> tuple[list[str], list[str]]:
        ...
        error_symbol = identify_error_symbol(indices, HelperNew._possible_error_symbols)
        if error_symbol != "":
    ...
    def _validate_key(self, keys) -> list[str]:
        ...
        symbol = identify_error_symbol(keys, HelperNew._possible_error_symbols)
        if symbol == "":
            raise ValueError(f"Key is not valid. Reminder: possible error symbols are {HelperNew._possible_error_symbols}")
