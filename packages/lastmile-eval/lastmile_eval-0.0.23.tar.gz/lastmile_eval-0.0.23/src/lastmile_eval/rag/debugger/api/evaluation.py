...
def run_queries_and_evaluate_outputs(
    project_id: str,
    trace_level_evaluators: Mapping[
        str,
        Callable[
            [Sequence[str], Sequence[str], Sequence[str] | None],
            Sequence[float],
        ],
    ],
    dataset_level_evaluators: Mapping[
    ...
    Special case of evaluation where
        ...
        - Evaluators take 3 inputs: queries, outputs, and ground truth (optional)
    """
    base_url = "https://lastmileai.dev"
    lastmile_api_token = _token(lastmile_api_token)

    outputs = evaluation_lib.run_rag_query_fn(rag_query_fn, queries)
    ...
    def evaluator_with_df_input(
        ...
        def _evaluator(df: pd.DataFrame) -> Sequence[float]:
            ...
            return evaluator(queries, output, ground_truth)
    ...
    def _make_df(queries: Sequence[str], outputs: Sequence[str]):
        df = pd.DataFrame(
            {
                "query": queries,
                "output": outputs,
