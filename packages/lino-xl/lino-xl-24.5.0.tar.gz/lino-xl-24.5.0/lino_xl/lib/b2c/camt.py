    ...
    def unique_id(self):
        ...
        if False:
            ...
            if self.start_date.year != year:
                raise Exception("%s starts %s and ends %s (different years)" %
                                (self, self.start_date, self.end_date))
        num = self.electronic_sequence_number
        if num is None:
            raise Exception("%s has no electronic_sequence_number" % self)
...
class CamtParser(object):
    ...
    def parse_transaction_details(self, ns, node, transaction):
        ...
        # remote party values
        party_type = 'Dbtr'
        party_type_node = node.xpath('../../ns:CdtDbtInd',
                                     namespaces={'ns': ns})
        if party_type_node and party_type_node[0].text != 'CRDT':
            party_type = 'Cdtr'
        party_node = node.xpath('./ns:RltdPties/ns:%s' % party_type,
                                namespaces={'ns': ns})
        if party_node:
            self.add_value_from_node(ns, party_node[0], './ns:Nm', transaction,
                                     'remote_owner')
            self.add_value_from_node(ns, party_node[0], './ns:PstlAdr/ns:Ctry',
                                     transaction, 'remote_owner_country')
            address_node = party_node[0].xpath('./ns:PstlAdr/ns:AdrLine',
                                               namespaces={'ns': ns})
            if address_node:
                transaction.remote_owner_address = '\n'.join(
                    [ln.text for ln in address_node])
        # Get remote_account from iban or from domestic account:
        account_node = node.xpath('./ns:RltdPties/ns:%sAcct/ns:Id' %
                                  party_type,
                                  namespaces={'ns': ns})
        if account_node:
            iban_node = account_node[0].xpath('./ns:IBAN',
                                              namespaces={'ns': ns})
            if iban_node:
                ...
                self.add_value_from_node(ns, account_node[0],
                                         './ns:Othr/ns:Id', transaction,
                                         'remote_account_other')

    def parse_transaction(self, ns, node, transaction):
        """Parse transaction (entry) node."""
        self.add_value_from_node(ns, node, './ns:BkTxCd/ns:Prtry/ns:Cd',
                                 transaction, 'txcd')
        self.add_value_from_node(ns, node, './ns:BkTxCd/ns:Prtry/ns:Issr',
                                 transaction, 'txcd_issuer')
        self.add_value_from_node(ns, node, './ns:BookgDt/ns:Dt', transaction,
                                 'booking_date')
        self.add_value_from_node(ns, node, './ns:ValDt/ns:Dt', transaction,
                                 'value_date')
        transaction.transferred_amount = self.parse_amount(ns, node)
        details_node = node.xpath('./ns:NtryDtls/ns:TxDtls',
                                  namespaces={'ns': ns})
        if details_node:
    ...
    def parse_statement(self, ns, node):
        ...
        self.add_value_from_node(ns, node, './ns:Id', statement,
                                 'statement_id')
        self.add_value_from_node(ns, node, './ns:LglSeqNb', statement,
                                 'legal_sequence_number')
        self.add_value_from_node(ns, node, './ns:ElctrncSeqNb', statement,
                                 'electronic_sequence_number')
        self.add_value_from_node(ns, node, './ns:Acct/ns:Ccy', statement,
                                 'local_currency')

        self.add_value_from_node(ns, node, './ns:Acct/ns:Nm', statement,
                                 'account_name')
        self.add_value_from_node(ns, node, './ns:Acct/ns:Ownr/ns:Nm',
    ...
    def check_version(self, ns, root):
        ...
        re_camt = re.compile(r'(^urn:iso:std:iso:20022:tech:xsd:camt.'
                             r'|^ISO:camt.)')
    ...
    def parse(self, data):
        ...
        if root is None:
            raise IOError('Not a valid xml file, or not an xml file at all.')
