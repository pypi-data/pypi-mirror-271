...
class VerifyUser(dd.Action):
    ...
    def html_response(self, ar, user):
        ...
        if user is not None:
            ...
            result.update(instance=f'<a href="/#{href}">Preferences ({user.get_full_name()})</a>')
        page_content = ""
        ...
        return HttpResponse(dd.plugins.jinja.renderer.jinja_env.get_template(
            'users/verification_response.html').render(**{"site": settings.SITE,
    ...
    def run_from_ui(self, ar, **kwargs):
        ...
        try:
            ...
            ar.set_response(goto_url=ar.get_permalink(rt.models.users.Me.detail_action, user))
...
class ResetPassword(dd.Action):
    ...
    def get_action_permission(self, ar, obj, state):
        if not allow_online_registration:
            # ResetPassword is not possible without verification code
            return False
        return super().get_action_permission(ar, obj, state)
