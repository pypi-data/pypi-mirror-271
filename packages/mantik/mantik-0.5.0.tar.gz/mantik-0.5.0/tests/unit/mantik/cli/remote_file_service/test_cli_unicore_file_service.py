...
def test_remove_file(fake_data_client, debug_caplog, cli_test_runner):
    result = cli_test_runner.invoke(
        ...
        [
            GROUP_NAME,
            "remove-file",
            "--id=fe0e8d90-8ea7-4011-bfd0-73e7c99b0900",
            "remote:/fake-dir/fake.file",
        ],
    )
...
def test_remove_file_without_reference(
    ...
    assert "remove_file remote:/fake-dir/fake.file" not in debug_caplog.text
...
def test_remove_directory_not_present_data_name(
    fake_data_client, debug_caplog, cli_test_runner
):
    result = cli_test_runner.invoke(
        main.cli,
    ...
    assert result.exit_code == 1
    assert (
        "A data repository with name: not-present-data-name was not found."
        in str(result)
    )
