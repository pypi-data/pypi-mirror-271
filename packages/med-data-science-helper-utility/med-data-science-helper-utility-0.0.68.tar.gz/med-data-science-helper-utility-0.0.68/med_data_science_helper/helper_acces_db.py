...
def get_gradiente(anio):
    url_gradiente = get_path_BD()+'\\19.Gradiente\\_data_\\GRADIENTE_'+str(anio)+'.csv'
    cols = ['COD_MOD','ANEXO','GRADIENTE']
    df_gradiente = pd.read_csv(url_gradiente ,usecols=cols, dtype={'COD_MOD':str,'ANEXO': 'uint8'})    
    return df_gradiente

def get_tiempos():
    url_gradiente = get_path_BD()+'\\18.Tiempos\\_data_\\TIEMPOS.xlsx'
    ...
    df_tiempos = pd.read_excel(url_gradiente,sheet_name="TIEMPOS" ,  dtype=dtypes_columns )
    return df_tiempos
