...
class Report:
    ...
    def __init__(self, metrics=None, header=True, per_label=False):
        ...
        all_metrics = ["precision-score-macro", "precision-score-micro", "average-precision-score-macro",
                       ...
                       "jaccard-score-macro", "jaccard-score-micro", "log-loss", "precision-score-per-label",
                       "average-precision-score-per-label", "recall-score-per-label", "f1-score-per-label",
    ...
    def to_csv(self, y_true, y_pred):
        ...
        if self.metrics_name is None:
            if self.header:
                header = ",".join(str(metric) for metric in self.metrics_value.keys())
                values = ",".join(str(value) for value in self.metrics_value.values())
        else:
            if self.header:
