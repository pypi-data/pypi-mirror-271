    ...
    class RunLogger:
        def __init__(self, run_id_dir, run_name, run_id_file_name="run_ids.json"):
            ...
            if run_name in run_ids:
                ...
                self.run = neptune.init_run(api_token= NeptuneLogger.api_token, project=NeptuneLogger.project_name, with_id=run_id, capture_stderr=True, async_no_progress_callback = stop_synchronization_callback)
    
            else:
                # Initialize a new run with the specified project name and run name
                self.run = neptune.init_run(api_token= NeptuneLogger.api_token, project=NeptuneLogger.project_name, name=run_name, capture_stderr=True, async_no_progress_callback = stop_synchronization_callback)
                # Get the run ID
