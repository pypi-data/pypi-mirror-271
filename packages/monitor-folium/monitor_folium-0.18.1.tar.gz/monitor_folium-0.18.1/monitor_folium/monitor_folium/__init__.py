...
if not _RELEASE:
    _component_func = components.declare_component(
        "mt_folium", url="http://localhost:3001"
    )
else:
    parent_dir = os.path.dirname(os.path.abspath(__file__))
    build_dir = os.path.join(parent_dir, "frontend/build")
    _component_func = components.declare_component("mt_folium", path=build_dir)
...
def folium_static(
    ...
    return mt_folium(fig, width=width, height=height, returned_objects=[])
