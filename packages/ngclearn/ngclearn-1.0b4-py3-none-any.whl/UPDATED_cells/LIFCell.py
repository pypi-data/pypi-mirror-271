...
@jit
def _modify_current(j, dt, tau_m):
    jScale = tau_m/dt
    return = j * jScale
...
def _dfv_internal(j, v, rfr, tau_m, refract_T, v_rest): ## raw voltage dynamics
    ...
    #dv_dt = (v_rest - v) * (dt/tau_m) + (j * mask)
...
@jit
def _emit_spikes(v, v_theta, v_thr):
    _v_thr = v_theta + v_thr
    return (v > _v_thr).astype(jnp.float32)

@jit
def _hyperpolarize(v, s, v_reset):
    _v = v * (1. - s) + s * v_reset
    return _v

@jit
def _update_refract_and_spikes(dt, rfr, s, mask, skey):
    ...
    if skey is not None: ## FIXME: this would not work for mini-batches!!!!!!!
        ...
        _s = s * (1. - m_switch) + rS * m_switch
    return _rfr, _s, raw_s
...
def run_cell(dt, j, v, v_thr, v_theta, rfr, skey, tau_m, R_m, v_rest, v_reset,
    ...
    if integType == 1:
        _v = step_rk2(0., v, v_params, _dfv, dt)
    else:
        _v = step_euler(0., v, v_params, _dfv, dt)

    s = _emit_spikes(_v, v_theta, v_thr) ## obtain action potentials
    _v = _hyperpolarize(_v, s, v_reset) ## hyper-polarize neuronal cells
    ## update refractory variables & post-process spikes/action potentials
    _rfr, s, raw_s = _update_refract_and_spikes(dt, rfr, spikes, mask, sticky_spikes)
...
class LIFCell(Component): ## leaky integrate-and-fire cell
    ...
    def __init__(self, name, n_units, tau_m, R_m, thr=-52., v_rest=-65., v_reset=60.,
                 tau_theta=1e7, theta_plus=0.05, refract_T=5., key=None,
                 integration_type="euler", one_spike=True, useVerboseDict=False,
    ...
    def advance_state(self, t, dt, **kwargs):
        ...
        self.current = _modify_current(self.current, dt, tau_m)
        ...
        self.voltage, self.spikes, raw_spikes, self.refract = \
            run_cell(dt, self.current, self.voltage, self.threshold,
                     ...
                     self.R_m, self.v_rest, self.v_reset, self.refract_T, self.intgFlag)
        if self.tau_theta > 0.:
