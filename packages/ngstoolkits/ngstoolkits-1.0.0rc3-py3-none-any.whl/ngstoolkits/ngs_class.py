...
class cpra():
    ...
    def get_suppot(self,bam_file, ref,coverflank=5):
        ...
        if self.muttype == "SNV":
            self.support_reads,self.support_readsID_list,self.cover_readsID_list = self.get_snv_support_reads( bam_file, ref,coverflank)
        elif self.muttype == "INS":
            self.support_reads,self.support_readsID_list,self.cover_readsID_list = self.get_ins_support_reads( bam_file, ref,coverflank)
        elif self.muttype == "DEL":
            self.support_reads,self.support_readsID_list,self.cover_readsID_list = self.get_del_support_reads( bam_file, ref,coverflank)

    @lru_cache
    def get_snv_support_reads(self, bam_file, ref,coverflank=5, mapq=20, baseq=20, overlaps=True, stepper="all", orphans=True):
        Read = namedtuple('Read', ['read_name', 'pair', 'strand'])
        support_reads = []
        cover_reads = []
        start_reads = {}
        EndSite = self.pos + len(self.ref)
        for pileup_column in bam_file.pileup(region=str(self.chrom) + ':' + str(self.pos) + '-' + str(self.pos),mapq=mapq , baseq = baseq,
        ...
        for pileup_column in bam_file.pileup(region=str(self.chrom) + ':' + str(EndSite) + '-' + str(EndSite),
                                            stepper=stepper, fastaFile=ref, max_depth=200000, **{"truncate": True}):
            if pileup_column.nsegments > 0:
                for pileup_read in pileup_column.pileups:
                    aln = pileup_read.alignment
    ...
    @lru_cache
    def get_ins_support_reads(self, bam_file, ref, coverflank=5, mapq=20, baseq=20, overlaps=True, stepper="all", orphans=True):
        support_reads = []
        cover_reads = []
        bam = {}
        EndSite = self.pos + len(self.ref)
        CoverStart = self.pos-coverflank
        CoverEnd = EndSite + coverflank
        insLength=len(self.alt)-len(self.ref)
        for pileup_column in bam_file.pileup(region=str(self.chrom) + ':' + str(self.pos) + '-' + str(self.pos), mapq=mapq, baseq=baseq, stepper=stepper, fastaFile=ref, max_depth=200000, **{"truncate": True}):
            if pileup_column.nsegments > 0:
                for pileup_read in pileup_column.pileups:
                    aln = pileup_read.alignment
    ...
    @lru_cache
    def get_del_support_reads(self, bam_file, ref, coverflank=5, mapq=20, baseq=20, overlaps=True, stepper="all", orphans=True):
        support_reads = []
        cover_reads = []
        bam = {}
        EndSite = self.pos + len(self.ref)
        CoverStart = self.pos-coverflank
        CoverEnd = EndSite + coverflank
        for pileup_column in bam_file.pileup(region=str(self.chrom) + ':' + str(self.pos) + '-' + str(EndSite), mapq=mapq , baseq = baseq,
                                            stepper=stepper, fastaFile=ref, max_depth=200000, **{"truncate": True}):
            if pileup_column.nsegments > 0:
                for pileup_read in pileup_column.pileups:
                    aln = pileup_read.alignment
