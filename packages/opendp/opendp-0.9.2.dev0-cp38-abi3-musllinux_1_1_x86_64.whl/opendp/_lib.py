...
def _load_library():
    ...
    if os.path.exists(lib_dir):
        # Map platform to library file name
        extension = {
            "darwin": "dylib",
            "linux": "so",
            "win32": "dll",
        ...
        lib_name = next((n for n in os.listdir(lib_dir) if n.endswith(extension)), None)
        
        if lib_name is None:
            raise Exception("Unable to find OpenDP shared library", sys.platform)
