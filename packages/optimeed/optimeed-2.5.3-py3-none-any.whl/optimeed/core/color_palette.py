...
def color_from_cmap(N, cmap):
    """Probe N evenly spaced colors from the colormap"""
    colors = np.linspace(0, 1, N)
    return cmap(colors)[:, :3]

def shuffle_colors(colors, seed=None):
    if seed is not None:
        print(seed)
        random.seed(seed)
...
def hex_to_rgb_255(value):
    value = value.lstrip('#')
    lv = len(value)
    return tuple(int(value[i:i + lv // 3], 16) for i in range(0, lv, lv // 3))

def hex_to_rgb_1(value):
    value = value.lstrip('#')
    lv = len(value)
    return tuple(int(value[i:i + lv // 3], 16)/255.0 for i in range(0, lv, lv // 3))


def adjust_lightness_1(color, amount=0.5):
    try:
        c = mc.cnames[color]
    except:
        c = color
...
def is_light_color(rgbColor, base=255):
    [r,g,b]=rgbColor
