...
class Real_OptimizationVariable(OptimizationVariable):
    ...
    def get_PhysToMaths(self, deviceIn):
        ...
        mid_value = (self.get_min_value() + self.get_max_value())/2
        if value is None:
            printIfShown("Initial variable {} is None. "
                         "The problem has been solved by using middle interval value: {}".format(self.get_attribute_name(), mid_value), SHOW_WARNING)
            return mid_value

        if not self.get_min_value() <= value <= self.get_max_value():
            printIfShown("Initial variable {} is out of bound with given value {}."
                         " The problem has been solved by using middle interval value: {}".format(self.get_attribute_name(), value, mid_value), SHOW_WARNING)
            return mid_value
        return value
...
class Integer_OptimizationVariable(OptimizationVariable):
    ...
    def get_PhysToMaths(self, deviceIn):
        value = super().get_PhysToMaths(deviceIn)
        mid_value = int((self.get_min_value() + self.get_max_value()) / 2.0)
        if value is None:
            printIfShown("Initial variable {} is None. "
                         "The problem has been solved by using middle interval value: {}".format(self.get_attribute_name(), mid_value), SHOW_WARNING)
            return mid_value
