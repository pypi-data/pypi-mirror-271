...
class Widget_lineDrawer(QtWidgets.QWidget):
    ...
    def paintEvent(self, event, painter=None):
        ...
        for key in self.dict_listOfLines:
            ...
            painter.drawLines([QtCore.QLineF(line[0]*ratio, -line[1]*ratio, line[2]*ratio, -line[3]*ratio) for line in self.dict_listOfLines[key]])

            # for line in self.dict_listOfLines[key]:
            #     painter.drawLine(QtCore.QPointF(line[0]*ratio, -line[1]*ratio), QtCore.QPointF(line[2]*ratio, -line[3]*ratio))
            thePen.setWidthF(0.5)
            painter.setPen(thePen)
            painter.drawLine(QtCore.QLineF(0, 0, x_max * ratio, 0))
            painter.drawLine(QtCore.QLineF(0, 0, 0, -y_max * ratio))
