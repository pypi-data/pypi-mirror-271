...
def show_accounts():
    with Session(Database(database_path).engine) as session:
        ...
        n = _array_map_field_max_len(data, "name")
        b = _array_map_field_max_len(data, "balance")
        s = 7  # Status
        c = _array_map_field_max_len(data, "comment")
        first_line = True
        for i in data:
            if first_line:
                header = f"| {'Name':^{n}} | {'Balance':^{b}} | {'Status':^{7}} | {'Comment':^{c}} |"
            ...
            print(
                f"| {i['name']:<{n}} | {i['balance']:>{b}}€ | {'Closed' if i['status'] else 'Open':<{s}} | {i['comment']:^{c}} |"  # noqa
            )
...
def _array_map_field_max_len(data, field):
    """
    Receives an array of dictionaries and finds the length of the biggest
     occurrence of field.
    """
    return max(len(item[field]) for item in data) if data else 0
