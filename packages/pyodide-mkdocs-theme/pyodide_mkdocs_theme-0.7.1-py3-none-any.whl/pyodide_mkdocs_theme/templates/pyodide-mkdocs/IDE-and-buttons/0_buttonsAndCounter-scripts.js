...
const play = (editorName) => withPyodideAsyncLock( async function(e){
    ...
    let [code, terminal, options, isOk] = await setupRuntimeAndTerminal(editorName);
    if(!isOk) return;
...
const validate = (editorName) => withPyodideAsyncLock( async function(e){
    ...
    if(!isOk){      // Threw AssertionError, otherwise already crashed

        if(CONFIG.decreaseAttemptsOnUserCodeFailure){
            const nAttemptsLeft = updateIdeCounter(editorName)
            if(unhideSolutionAndRem(editorName, nAttemptsLeft, false)){
                finalMsg = enhanceFailureMsg(editorName)
            }
        }
        return
    }
    ...
    try{
        ...
        stdErr = isOk ? await runPythonCodeWithOptions(code, terminal, options, true)
                      : "nope..."

        decrease_count = CONFIG.decreaseAttemptsOnUserCodeFailure && stdErr && isOk
