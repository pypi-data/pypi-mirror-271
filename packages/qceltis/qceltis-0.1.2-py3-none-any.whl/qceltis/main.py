...
from qceltis.mod.mzml_extract import calculate_idfree_metrics
from qceltis.mod.idbased_metrics import calculate_idbased_metrics
from qceltis.mod.general_functions import check_path, check_file, check_grouping_file, get_grouping_dict, check_samples, int_range, check_duplicates, get_overall_qc_status, groupname
...
def main():
    ...
    if all_report_params:
        #env = Environment(loader=FileSystemLoader(str("qceltis/templates")))
        #template = env.get_template(str("report_template.html"))
        
        #env = Environment(loader=PackageLoader("qceltis"),autoescape=select_autoescape())
        #env = Environment(loader=PackageLoader('qceltis', 'templates'))
