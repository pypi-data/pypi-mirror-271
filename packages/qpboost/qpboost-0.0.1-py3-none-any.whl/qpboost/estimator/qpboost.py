...
from .base import _QPBASE
from ..utils import check_inputs
from ..solver import GurobiSolver


class QPBOOSTClassifier(_QPBASE):
    """
    ...
    def _get_T(self, x, T, vec_y, fit_tree, betas=None):
        no_improvement = True
        preds = fit_tree.predict(x).astype(np.int8)

        label_vector = np.full(
            (
                x.shape[0],
                self.k_,
            ),
        ...
        label_vector[np.arange(len(preds)), preds] = 1

        fill_ahat = np.multiply(vec_y, label_vector).sum(axis=1).reshape(-1, 1) * (
            (self.k_ - 1.0) / self.k_
        )

        red_cost = float("inf") if betas is None else np.dot(fill_ahat.squeeze(), betas)

        if red_cost > 0:
            T = fill_ahat if T is None else np.hstack((T, fill_ahat))
            no_improvement = False

        return T, no_improvement
    ...
    def fit(self, x: ArrayLike, y: ArrayLike, sample_weight: ArrayLike | None = None):
        ...
        self.decision_trees_.append(fit_tree)
        T = None

        self._get_class_infos(y)
        vec_y = self._preprocess(y)
        T, _ = self._get_T(x, T, vec_y, fit_tree)

        betas = self.solver(T=T, sample_weight=sample_weight, primal=False)
        ...
        for _ in range(self.max_rmp_calls):
            ...
            T, no_improvement = self._get_T(x, T, vec_y, fit_tree, betas)
