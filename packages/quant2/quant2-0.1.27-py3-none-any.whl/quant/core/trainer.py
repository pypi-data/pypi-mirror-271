...
def train_model():
    ...
    for cur_epoch in range(start_epoch, cfg.OPTIM.MAX_EPOCH):
        ...
        if dist.is_main_proc():
            nni_metric = {
                "default": 100.0 - min(test_err, ema_err),
                "test_err": test_err,
                "ema_err": ema_err,
                "checkpoint": file,
            }
            if nni_metric["default"] > nni_metric_best["default"]:
                nni_metric_best = nni_metric
            nni.report_intermediate_result(nni_metric)
    # nni report
    if dist.is_main_proc():
        nni.report_final_result(nni_metric_best)
    logger.info(logging.dump_log_data(nni_metric_best, "nni_metric_best"))
