...
class NpzDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path, split, fixed=False):
        assert pathmgr.exists(data_path), "Data path '{}' not found".format(data_path)
        self._data_path, self._split, self._fixed = pathlib.Path(data_path), split, fixed
    ...
    def _construct_dataset(self):
        ...
        self.preprocess_kwargs["context_length"] = cfg.TRAIN.CONTEXT_LENGTH
    ...
    def _prepare_im(self, im_path):
        ...
        if self._fixed:
            curr_time = (time_low + time_high) * 0.5
