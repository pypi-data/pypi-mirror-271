...
def get_params(**kwargs):
    ...
    context_length = kwargs.get("context_length", 18)

    point_start = kwargs.get("point_start", 0.5)
    point_stop = kwargs.get("point_stop", 21.1)
...
def get_sample(curr_time, atime, aodds, mat, **kwargs):
    ...
    total_time = 90 if time_range[1] > 45.1 else 45
    ...
    oth = np.asarray([((1 <= mat) & (mat <= b)).sum(axis=0).tolist() + [a, b, curr_time, total_time - curr_time] for a, b in slices], dtype=np.float32)
