...
class RshipExecClient:
    ...
    async def pulse_emitter(self, emitter_id: str, data: Any):
        pulse = Pulse(name="", emitter_id=emitter_id, data=data)
        await self.set_data(pulse, 'Pulse')

    async def set_instance_offline(self, instance_id: str):
        instance = self.instances.get(instance_id)
        if instance:
            instance.status = InstanceStatusEnum.Unavailable
            await self.set_data(instance, 'Instance')

    async def set_target_offline(self, target_id: str, instance_id: str):
        target = self.targets.get(target_id)
        if target:
            await self.set_target_status(target_id, instance_id, TargetStatusEnum.Offline)

    async def set_target_status(self, target_status: TargetStatus):
        self.target_statuses[target_status.id] = target_status
    ...
    async def save_target(self, target: Target):
        if target.id not in self.targets or self.targets[target.id].hash != target.hash:
    ...
    async def save_emitter(self, emitter: Emitter):
        if emitter.id not in self.emitters:
