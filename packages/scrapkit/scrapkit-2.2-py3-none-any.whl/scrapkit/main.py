...
def saveFile(content, filename):
    try:
        if isinstance(content, list):  # Check if content is a list
            content = '\n'.join(content)  # Convert list to string with each item on a new line
        with open(filename, "w", encoding="utf-8") as file:
            file.write(content)
...
def getAttributeFromElements(url, tag_name, attribute_name):
    response = requests.get(url)
    if response.status_code == 200:
        soup = BeautifulSoup(response.text, 'html.parser')
        ...
        print(f"Failed to fetch attributes '{attribute_name}' from elements '{tag_name}'. Status code:", response.status_code)
        return None

def scrapeMultiplePages(base_url, num_pages):
    all_links = []
...
def cleanText(text):
    # Example cleaning operations: removing HTML tags and extra whitespace
