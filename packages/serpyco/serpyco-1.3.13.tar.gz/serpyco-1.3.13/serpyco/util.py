...
class _DataClassParams(object):
    ...
    def __init__(
        ...
        self.arguments = tuple(
            ...
            for t_ in typing_inspect.get_generic_bases(type_) + (type_,)
            for arg in typing_inspect.get_args(t_, evaluate=True)
        )
        self.parameters = tuple(
            param
            for t_ in typing_inspect.get_generic_bases(type_) + (type_,)
            for param in typing_inspect.get_parameters(typing_inspect.get_origin(t_))
        )
