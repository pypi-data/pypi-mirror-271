...
# region fsub_filter
async def fsub_filter(_, c: "pyrogram.Client", m: Message):
    if m.from_user.is_self:
        return True
    if not c._statusFsub:
        return True
    if len(c._forceSub) == 0:
        return True
    devs = c._devs
    sudo = c._sudo
    links = []
    member = None
    for v in c._forceSub:
        links.append(v)
...
fsub = create(fsub_filter)
"""Filter messages generated by you fsub."""
