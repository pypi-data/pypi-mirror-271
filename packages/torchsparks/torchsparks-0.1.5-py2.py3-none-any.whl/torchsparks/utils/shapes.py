...
def shape(
    ...
    def shape_recursively(depth, depth_inputs):
        if isinstance(depth_inputs, (int, float, bool, bytes, bytearray)):
            return [], torch.tensor(1, dtype=torch.int32) if need_len else None
        elif isinstance(depth_inputs, torch.Tensor):
...
def pad(
        ...
        if len_dimensions == "different":
            def len_dimension_recursively(depth, depth_data):
                if isinstance(depth_data, (bytes, bytearray)):
                    return depth
                elif isinstance(depth_data, (int, float, bool)):
                    return depth - 1
    ...
    def pad_recursively(depth, depth_data, depth_lens):
        if isinstance(depth_data, (int, float, bool, bytes, bytearray)):
            depth_data_type = type(depth_data)

            if depth_data_type in (bytes, bytearray):
                depth_data = list(depth_data)
                return (torch.tensor(depth_data, dtype=data_dtype or torch.uint8, device=data_device),
                        torch.tensor(len(depth_data), dtype=len_dtype, device=len_device))

            return_depth_lens = torch.tensor(1, dtype=len_dtype, device=len_device)

            depth_data_dtype = data_dtype
            if depth_data_dtype is None:
                if depth_data_type == float:
                    depth_data_dtype = torch.float32
                elif depth_data_type == bool:
                    depth_data_dtype = torch.bool
                elif depth_data_type == int:
                    depth_data_dtype = torch.int32
                else:
                    raise ValueError(f"Invalid value for `depth_data`: {depth_data}")

            return torch.tensor(depth_data, dtype=depth_data_dtype, device=data_device), return_depth_lens
        elif isinstance(depth_data, torch.Tensor):
    ...
    len_results = (len_results[0] if len_results.ndim else len_results.item()) if need_len and len_results is not None else None
