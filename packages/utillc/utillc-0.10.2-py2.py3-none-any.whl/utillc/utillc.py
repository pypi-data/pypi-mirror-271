...
def EKON(*args,n=INFO,pref="", color=False, check=False, withprint=True, llevel=0, pattern="EKOX", opt={}):
    ...
    """
    if len(args) == 1 :
        EKOX(args[0], n=n, llevel=llevel+1, pattern="EKON", istuple=len(args)>1, opt=opt)    
    else :
        EKOX(args, n=n, llevel=llevel+1, pattern="EKON", istuple=len(args)>1, opt=opt)            
...
def EKOT(*x,n=INFO,pref="", color=False, llevel=0, opt={} ):
    ...
    if (ff[4] != None and n >= el()) :
        loc = str(ff[1]) +  ':' + str(ff[2])
        r += print1( loc +  ': ' + tt(loc) + lab(n) + '=' + pref + ', '.join(map(str,x)) + '.', color=color, opt=opt)
    return r
