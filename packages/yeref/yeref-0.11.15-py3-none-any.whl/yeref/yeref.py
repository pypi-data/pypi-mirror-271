...
<body>
    <div class="container">
        ...
        <div class="address-collection">
            ...
            <div class="address-collection-input"><input type="text" id="address-collection-input-id" minlength="48"
                    maxlength="48" placeholder="для проверки NFT">
            </div>
        </div>
    ...
    <script>
        ...
        document.querySelector('[data-tc-button="true"]').style.minWidth = window.getComputedStyle(document.getElementById('address-minter-input-id')).getPropertyValue('width')
        console.log('width ', window.getComputedStyle(document.getElementById('address-minter-input-id')).getPropertyValue('width'))

        tg.MainButton.onClick(async () => {
            console.log('Кнопка была нажата');
            document.querySelector('[data-tc-button="true"]').style.minWidth = window.getComputedStyle(document.getElementById('address-minter-input-id')).getPropertyValue('width')
            let cost = document.getElementById("invoice-input").value
            let currency = document.getElementById("subscribe-datalist-id").value
            let period = document.getElementById("period").value
            let minter = document.getElementById("address-minter-input-id").value
            let collection = document.getElementById("address-collection-input-id").value
            ...
            if (currency !== "TON" && minter === "") {
                tg.showAlert("При указании других валют необходимо заполнить адрес коллекии монет await fetchData(getUrl);")
                return
            }
            
            console.log('fetch data to back: console.log(cost, currency, period, minter, collection, owner)')
            console.log(cost, currency, period, minter, collection, owner)
        ...
        async function sendRequest(method, url, body = null) {
            ...
            try {
                ...
                throw err;  // Повторное выбрасывание ошибки для ее перехвата вызывающим кодом
            }
        }
        async function fetchData(url) {
            try {
...
<body>
    <div class="container">
        ...
        <div class="subscribe">
            <div class="subscribe-ft">
                <div class="subscribe-ft-text">Подписка: 1.2 TON в месяц</div>
                <div id="ft-send">Отправить 1.2 TON</div>
                <a class="optional" id="ft-link"
                    href="https://tonviewer.com/EQDIePxrqM9RRikDVLR6SMomjjz3apnp0rzWtdQMJDQNm2p5">нет активных
                    подписок</a>
                <!-- <a class="optional" id="ft-link" href="https://tonviewer.com/EQDIePxrqM9RRikDVLR6SMomjjz3apnp0rzWtdQMJDQNm2p5">Подписка
                    активирована до
                    12/01/24</a> -->
            </div>
            <div class="subscribe-nft">
                <!-- ✔️ -->
                <div class="subscribe-nft-text">Проверка наличия NFT</div>
                <div id="nft-get">Приобрести NFT</div>
                <div id="nft-check">Проверить наличие NFT</div>
                <input type="text" id="nft-address" minlength="48" maxlength="48" placeholder="адрес полученного NFT">
                <a class="optional" id="nft-link"
                    href="https://tonviewer.com/EQDIePxrqM9RRikDVLR6SMomjjz3apnp0rzWtdQMJDQNm2p5">нет
                    необходимого NFT</a>
                <!-- <a class="optional" id="nft-link" href="https://tonviewer.com/EQDIePxrqM9RRikDVLR6SMomjjz3apnp0rzWtdQMJDQNm2p5">NFT уже
                    получен</a> -->
            </div>
        </div>
        <div class="sticker">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {
            ...
            document.getElementById("ft-link").addEventListener("click", async (event) => {
                event.preventDefault()
                tg.openLink(`https://tonviewer.com/EQDIePxrqM9RRikDVLR6SMomjjz3apnp0rzWtdQMJDQNm2p5`, { try_instant_view: true })
            })

            document.getElementById("nft-link").addEventListener("click", async (event) => {
                event.preventDefault()
                tg.openLink(`https://tonviewer.com/EQDIePxrqM9RRikDVLR6SMomjjz3apnp0rzWtdQMJDQNm2p5`, { try_instant_view: true })
            })
        })
...
async def check_webapp_hash(init_data, TOKEN_BOT, BOT_TOKEN_MAIN=None):
    ...
    try:
        ...
        TOKEN_BOT = TOKEN_BOT if BOT_TOKEN_MAIN is None or BOT_TOKEN_MAIN == '' else BOT_TOKEN_MAIN
