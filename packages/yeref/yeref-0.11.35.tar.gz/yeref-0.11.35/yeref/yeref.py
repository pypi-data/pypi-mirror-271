...
<body>
    <div class="container">
        <div class="header">
            <div class="subscribe-text"><a id="link" href="{0}">{1}</a></div>
            <div id="connect"></div>
        </div>
        <div class="subscribe">
            <div class="subscribe-ft">
                <div class="subscribe-ft-text">Подписка: {2} {3} {4}</div>
                <div id="ft-send">Отправить {2} {3}</div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            if (cost === "0" || cost === "0.0") {{
                document.querySelector('.subscribe-ft').style.display = 'none'
            }}
            else {{
                console.log('tyu')
            }}
            ...
            if (collection === "" ) {{
                document.querySelector('.subscribe-nft').style.display = 'none'
            }}
            ...
            document.getElementById("nft-link").addEventListener("click", async (event) => {{ tg.openLink(`https://tonviewer.com/${{owner}}`, {{ try_instant_view: true }}) }})
        }})
        
        async function fetchData(url) {{
            try {{ const response = await fetch(url); console.log('response:', response)
            }} catch (error) {{ console.log('Error fetching data:', error); }}
        }}
        tonConnectUI.onStatusChange(async (wallet) => {{ console.log('onStatusChange', wallet); if (tonConnectUI.connected) {{ owner = tonConnectUI.account.address; }} }})
        tg.MainButton.onClick(async () => {{ tg.openTelegramLink("{0}") }})
