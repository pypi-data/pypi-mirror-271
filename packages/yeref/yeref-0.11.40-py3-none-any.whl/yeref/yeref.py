...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            console.log('1 so')
            document.getElementById("invoice-input").value = cost;
        ...
        tg.MainButton.onClick(async () => {{
            document.querySelector('[data-tc-button="true"]').style.minWidth = window.getComputedStyle(document.getElementById('address-minter-input-id')).getPropertyValue('width')
            cost = document.getElementById("invoice-input").value
        ...
        async function sendRequest(method, url, body = null) {{
            try {{  const response = await fetch(url, {{ method, body: JSON.stringify(body), {{ 'Content-Type': 'application/json' }} }})
