    ...
    <div class="container">
        ...
        <div class="subscribe">
            <div class="subscribe-ft">
                <div class="subscribe-ft-text">{11} {4} {5} {6}</div>
    ...
    <script>
        ...
        document.getElementById('ft-send').addEventListener('click', async () => {{
            try {{
                ...
                let cell = new tonweb.boc.Cell();
                cell.bits.writeUint(0, 32);
                cell.bits.writeString(`${{tid}}_${{tg.initDataUnsafe['user']['id']}}`);
                ...
                console.log(parseFloat(amount)*1000000000)
                console.log(tonweb.utils.toNano("1.0").toString())
                let message = [{{
                    address: owner,
                    amount: tonweb.utils.toNano("1.0").toString(),
                    payload: payload,
                }}]
                console.log(message);
                
                console.log(`https://tonviewer.com/${{owner}}`);
                const result = await tonConnectUI.sendTransaction({{ validUntil: Math.round(Date.now() / 1000) + 60, messages: message, }});
            }} catch (error) {{
