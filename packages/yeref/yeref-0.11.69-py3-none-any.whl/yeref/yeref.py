...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            document.querySelector('.subscribe-ft').style.display = "{9}"
            document.querySelector('.subscribe-nft').style.display = "{10}"
            tg.MainButton.show().setParams({{ text: "{19}", is_active: true, is_visible: true }})
            ...
            let responseData = await sendRequest("POST", "/upaydt", {{ 'tid': tid, 'initData': tg.initData }})
            if (Object.keys(responseData).length > 0) {{
                document.getElementById('ft-link').textContent = responseData['USER_PAYDT']
                document.getElementById('nft-link').textContent = responseData['USER_PAYNFT']
                console.log(responseData['USER_PAYDT'])
                
                let handleClick = async (event) => {{ tg.openLink(`https://tonviewer.com/${{owner}}`, {{ try_instant_view: true }}) }}
                
                if (responseData['USER_PAYDT'].includes('-')) {{
                    document.getElementById("ft-link").removeEventListener("click", handleClick)
                    document.getElementById('ft-send').textContent = '✔️ {12} {4} {5}'
                    document.getElementById('ft-send').disabled = true
                }}
                else {{
                    document.getElementById("ft-link").addEventListener("click", handleClick)
