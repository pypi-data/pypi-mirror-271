...
<head>
    ...
    <style>
        ...
        *::after {{
        ...
        .subscribe-link, .subscribe-text, .subscribe-datalist, .subscribe-input, .address-minter-input, .address-collection-input {{ width: 50%; }}
...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{            
            ...
            document.querySelector('.address-minter-text').textContent = "{14}"
            document.getElementById('address-minter-input-id').setAttribute('placeholder', "{15}")
            ...
            document.getElementById("address-minter-input-id").value = minter;
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            if (Object.keys(upayinfo).length > 0) {{
                ...
                console.log(upayinfo['nft'])
                console.log(document.getElementById('nft-address').value)
                document.getElementById('nft-address').value = upayinfo['nft']
                
                if (upayinfo['is_upaydt']) {{
                    document.getElementById('ft-send').textContent = '✔️ {12} {4} {5}'
                    document.getElementById('ft-send').disabled = true
                }}
                else {{
                    document.getElementById('ft-send').textContent = '{12} {4} {5}'
                    document.getElementById('ft-send').disabled = false
                }}
                
                if (upayinfo['is_upaynft']) {{
                    document.getElementById('nft-get').textContent = '✔️ {15}'
                    document.getElementById('nft-get').disabled = true
                }}
                else {{
                    document.getElementById('nft-get').textContent = '{15}'
                    document.getElementById('nft-get').disabled = false
                }}
            }}
        }})
        ...
        document.getElementById('ft-send').addEventListener('click', async () => {{
            try {{
                console.log('click ft-send', document.getElementById('ft-send').disabled)
                ...
                await sendRequest("POST", "/upayft", body)
        ...
        document.getElementById('nft-check').addEventListener('click', async () => {{
            ...
            if (nftAddress !== "") {{ 
                let get_nft_data = await sendRequest("POST", "/get_nft_data", {{ 'tid': tid, 'address': tonConnectUI.account.address, 'collection': collection, 'nftAddress': nftAddress, 'initData': tg.initData }})
                console.log('before upayinfo', upayinfo)
                console.log('get_nft_data', get_nft_data)
                upayinfo['is_upaynft'] = get_nft_data['is_upaydt']
                upayinfo['upaynft'] = get_nft_data['upaynft']
                upayinfo['nft'] = get_nft_data['nft']
                console.log('upayinfo', upayinfo)
                return
            }}
        ...
        async function getNft(nftAddress) {{
            let resGetNft = ""
            try {{
                console.log('getNft', nftAddress);
                // const tonweb = new window.TonWeb(new TonWeb.HttpProvider('https://testnet.toncenter.com/api/v2/jsonRPC'));
                const tonweb = new window.TonWeb(new TonWeb.HttpProvider('https://toncenter.com/api/v2/jsonRPC'));
                const result = await tonweb.provider.call(nftAddress, 'get_nft_data', [])
                console.log('GET method result:', result)
                
                console.log(0)
                console.log(result.stack[2][1])
                payload = tonweb.utils.bytesToBase64(cellBoc);
                payload = tonweb.utils.base64ToBytes(result.stack[2][1]);
                
                const cell = tonweb.boc.Cell.fromBoc(Buffer.from(result.stack[2][1], 'base64'))[0]
                console.log(1)
                let addrColUser = tonweb.utils.Address(cell.bits.readAddress())
                console.log('a1', addrColUser.toRawString())
                console.log(2)
                let addrColMy = tonweb.utils.Address(collection)
                console.log('a2', addrColMy.toRawString())
                console.log(3)
            }} catch (error) {{
                console.error('getNft:', error);
            }} finally {{
                return resGetNft
            }}
        }}
