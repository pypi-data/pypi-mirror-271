...
<body>
    ...
    <script>
        ...
        document.getElementById('subscribe-datalist-id').addEventListener('input', async () => {{
            console.log('Введен текст:', document.getElementById('subscribe-datalist-id').value);
            let val = document.getElementById('subscribe-datalist-id').value
            
            if (val !== 'TON') {{
                document.querySelector('.address-minter').style.display = 'flex'
            }}
            else {{
                document.querySelector('.address-minter').style.display = 'none'
            }}
            
            document.querySelector('.address-minter-text').textContent = `${{addrCollectText}} ${{val}}:*`
        }})
    ...
    <script>
        ...
        document.getElementById('nft-check').addEventListener('click', async () => {{
            ...
            if (nftAddress !== "") {{ 
                ...
                upayinfo['is_upaynft'] = (get_nft_data['is_upaynft'] !== null) ? get_nft_data['is_upaynft'] : 0
                upayinfo['upaynft'] = (get_nft_data['upaynft'] !== null) ? get_nft_data['upaynft'] : ""
                upayinfo['nft'] = (get_nft_data['nft'] !== null) ? get_nft_data['nft'] : ""
                console.log('upayinfo', upayinfo)
                return
            }}
